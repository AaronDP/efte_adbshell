...
EDirectory::EDirectory(int         createFlags,
                       EModel    **ARoot,
                       const char *aPath) : EList(createFlags, ARoot, aPath) {
  char XPath[MAXPATH];
  ...
  if (ExpandPath(aPath, XPath, sizeof(XPath)) == -1) strcpy(XPath, "/");
  Slash(XPath, 1);
  Path = strdup(XPath);
  RescanList();
}
...
void EDirectory::DrawLine(PCell B, int Line, int Col, ChColor color, int Width) {
  ...
  if (Files && (Line >= 0) && (Line < FCount)) {
    ...
    if (Size >= 10 * 1024 * 1024) {
      ...
      if (Size >= 1024 * 1024 *
          1024) sprintf(SizeStr, "%7ldG", (long)(Size / 1024 / 1024));
      else if (Size >= 1024 * 1024) sprintf(SizeStr, "%7ldM", (long)(Size / 1024));
      else sprintf(SizeStr, "%7ldK", (long)Size);
    } else sprintf(SizeStr, "%8ld", (long)Size);
    ...
    if (Col < int(strlen(s))) MoveStr(B, 0, Width, s + Col, color, Width);
  }
}
...
int _LNK_CONV FileNameCmp(const void *a, const void *b) {
  ...
  if (!(A->Type() == fiDIRECTORY) && (B->Type() == fiDIRECTORY)) return 1;
...
void EDirectory::RescanList() {
  ...
  // we don't want any special information about symbolic links, just to browse
  // files
  ...
  while (rc == 0) {
    ...
    if (strcmp(fi->Name(), ".") != 0) {
      ...
      if ((fi->Type() == fiDIRECTORY) &&
          (strcmp(fi->Name(), "..") != 0)) DirCount++;
      Count++;
      FCount++;
    } else delete fi;
    rc = ff->FindNext(&fi);
...
int EDirectory::ExecCommand(int Command, ExState& State) {
  ...
  case ExDirSearchNext:
    ...
    // Find next matching file, search is case in-sensitive while sorting is
    // sensitive
    if (SearchLen) {
  ...
  case ExDirSearchPrev:
    ...
    // Find prev matching file, search is case in-sensitive while sorting is
    // sensitive
    if (SearchLen) {
...
int EDirectory::GetMatchForward(int start) const {
  if (start == 0) {
    ...
    for (int i = 0; i < FCount; i++) {
      ...
      if (strnicmp(SearchName, fname, SearchLen) == 0) return i;
    }
  }
  ...
  for (int i = start; i < FCount; i++) {
    ...
    for (int j = 0; fname[j]; j++) {
      if (strnicmp(SearchName, fname + j, SearchLen) == 0) return i;
    }
  }
  return -1;
}
...
int EDirectory::RescanDir() {
  ...
  if ((Row >= 0) && (Row < FCount)) strcpy(CName, Files[Row]->Name());
  Row = 0;
  RescanList();
...
int EDirectory::FmMvFile(const char *Name) {
  ...
  if (View->MView->Win->GetStr("New file name", sizeof(Dir), Dir,
                               HIST_PATH) == 0) {
    return 0;
  }
...
int EDirectory::FmMkDir() {
  ...
  if (View->MView->Win->GetStr("New directory name", sizeof(Dir), Dir,
                               HIST_PATH) == 0) {
    return 0;
  }
  ...
  defined(__WATCOM_CPLUSPLUS__)
  int status = mkdir(Dir2);
#else // if defined(MSVC) || defined(BCPP) || defined(WATCOM) ||
      // defined(__WATCOM_CPLUSPLUS__)
  int status = mkdir(Dir2, 509);
#endif // if defined(MSVC) || defined(BCPP) || defined(WATCOM) ||
       // defined(__WATCOM_CPLUSPLUS__)
...
int EDirectory::ChangeDir(ExState& State) {
  ...
  if (State.GetStrParam(View, Dir, sizeof(Dir)) == 0) {
    ...
    if (View->MView->Win->GetStr("Set directory", sizeof(Dir), Dir,
                                 HIST_PATH) == 0) return 0;
  }

  if (ExpandPath(Dir, Dir2, sizeof(Dir2)) == -1) return 0;
