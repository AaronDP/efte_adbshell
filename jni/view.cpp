...
EView::EView(EModel *AModel) {
  ...
  if (Model) Model->CreateViewPort(this);
}
...
EView::~EView() {
  if (Next != this) {
    ...
    if (ActiveView == this) ActiveView = Next;
  } else ActiveView = 0;

  if (MView) MView->View = 0;

  if (Model) Model->RemoveView(this);

  if (Port) delete Port;
}

int EView::CanQuit() const {
  if (Model) return Model->CanQuit();
  else return 1;
}

void EView::FocusChange(int GetFocus) {
  if (GetFocus) {
    if (Model->View && Model->View->Port) Model->View->Port->GetPos();
    ...
    if (Model) {
      ...
      if (Model->View && Model->View->Port) Model->View->Port->StorePos();
    }
  }
}

void EView::Resize(int Width, int Height) {
  if (Port) Port->Resize(Width, Height);
}
...
void EView::SelectModel(EModel *AModel) {
  if (Model != AModel) {
    if (Model) FocusChange(0);
    SetModel(AModel);

    if (Model) FocusChange(1);
  }
}
...
void EView::HandleEvent(TEvent& Event) {
  if (Model) Model->HandleEvent(Event);

  if (Port) Port->HandleEvent(Event);
  ...
  if (Event.What == evCommand) {
    ...
    case cmDroppedFile: {
      ...
      if (IsDirectory(file)) OpenDir(file);
...
void EView::UpdateView() {
  if (Port) Port->UpdateView();
}

void EView::RepaintView() {
  if (Port) Port->RepaintView();
}

void EView::UpdateStatus() {
  if (Port) Port->UpdateStatus();
}

void EView::RepaintStatus() {
  if (Port) Port->RepaintStatus();
}
...
void EView::DeleteModel(EModel *M) {
  ...
  while (V) {
    ...
    if (M1 == M) {
      if (M->Next != M) V->SelectModel(M->Next);
      else V->SelectModel(0);
    }
    V = V->Next;

    if (V == ActiveView) break;
  }
  delete M;
  SetMsg(0);
}
...
int EView::FilePrev() {
  if (Model) {
    ...
    if (IgnoreBufferList && n &&
        (n->GetContext() == CONTEXT_BUFFERS)) n = n->Prev;
    SelectModel(n);
    return 1;
  }
  return 0;
...
int EView::FileNext() {
  if (Model) {
    ...
    if (IgnoreBufferList && n &&
        (n->GetContext() == CONTEXT_BUFFERS)) n = n->Next;
    SelectModel(n);
    return 1;
  }
  return 0;
...
int EView::SwitchTo(ExState& State) {
  ...
  if (State.GetIntParam(this, &No) == 0) {
    ...
    if (MView->Win->GetStr("Obj.Number", sizeof(str), (char *)str,
                           0) == 0) return 0;
...
int EView::FileOpen(ExState& State) {
  ...
  if (State.GetStrParam(this, FName, sizeof(FName)) == 0) {
    ...
    if (MView->Win->GetFile("Open file", sizeof(FName), FName, HIST_PATH,
                            GF_OPEN) == 0) return 0;
  }

  if (strlen(FName) == 0) return 0;

  if (IsDirectory(FName)) return OpenDir(FName);
...
int EView::FileOpenInMode(ExState& State) {
  ...
  if (State.GetStrParam(this, Mode,
                        sizeof(Mode)) ==
      0) if (MView->Win->GetStr("Mode", sizeof(Mode), Mode,
                                HIST_SETUP) != 1) return 0;
  ...
  if (State.GetStrParam(this, FName,
                        sizeof(FName)) ==
      0) if (MView->Win->GetFile("Open file", sizeof(FName), FName, HIST_PATH,
                                 GF_OPEN) == 0) return 0;
...
int EView::SetPrintDevice(ExState& State) {
  ...
  if (State.GetStrParam(this, Dev,
                        sizeof(Dev)) ==
      0) if (MView->Win->GetStr("Print to", sizeof(Dev), Dev,
                                HIST_SETUP) == 0) return 0;
...
void EView::Msg(int level, const char *s, ...) {
  ...
  if (level != S_BUSY) SetMsg(msgbuftmp);
}
...
void EView::SetMsg(const char *msg) {
  ...
  if (msg && strlen(msg)) CurMsg = strdup(msg);
  ...
  if (CurMsg && msg && MView) {
    ...
    if (MView->IsActive()) SColor = hcStatus_Active;
    else SColor = hcStatus_Normal;

    MoveChar(B, 0, Cols, ' ', SColor, Cols);
    MoveStr(B, 0, Cols, CurMsg, SColor, Cols);

    if (MView->Win->GetStatusContext() == MView) MView->ConPutBox(0,
                                                                  Rows - 1,
                                                                  Cols,
                                                                  1,
                                                                  B);
...
int EView::ViewRoutines(ExState& /*State*/) {
  ...
  if (M->GetContext() != CONTEXT_FILE) return 0;
...
int EView::DirOpen(ExState& State) {
  ...
  if (State.GetStrParam(this, Path, sizeof(Path)) == 0)
    if (GetDefaultDirectory(Model, Path, sizeof(Path)) == 0) return 0;
...
int EView::OpenDir(const char *Path) {
  ...
  if (ExpandPath(Path, XPath, sizeof(XPath)) == -1) return 0;
  ...
  if (dir == 0) dir = new EDirectory(0, &ActiveModel, XPath);
  SelectModel(dir);
  return 1;
}
...
int EView::Compile(ExState& State) {
  ...
  if (State.GetStrParam(this, Command, sizeof(Command)) == 0) {
    if (Model->GetContext() == CONTEXT_FILE) {
      ...
      if (BFS(B, BFS_CompileCommand) != 0) strcpy(Cmd, BFS(B, BFS_CompileCommand));
    }

    if (Cmd[0] == 0) strcpy(Cmd, CompileCommand);

    if (MView->Win->GetStr("Compile", sizeof(Cmd), Cmd,
                           HIST_COMPILE) == 0) return 0;

    strcpy(Command, Cmd);
  } else {
    if (MView->Win->GetStr("Compile", sizeof(Command), Command,
                           HIST_COMPILE) == 0) return 0;
  }
...
int EView::RunCompiler(ExState& State) {
  ...
  if (State.GetStrParam(this, Command, sizeof(Command)) == 0) {
    if (Model->GetContext() == CONTEXT_FILE) {
      ...
      if (BFS(B,
              BFS_CompileCommand) != 0) strcpy(Command, BFS(B, BFS_CompileCommand));
    }

    if (Command[0] == 0) strcpy(Command, CompileCommand);
  }
  return Compile(Command);
}
...
int EView::CompilePrevError(ExState& /*State*/) {
  if (CompilerMsgs != 0) return CompilerMsgs->CompilePrevError(this);

  return 0;
}

int EView::CompileNextError(ExState& /*State*/) {
  if (CompilerMsgs != 0) return CompilerMsgs->CompileNextError(this);

  return 0;
}

int EView::ShowVersion() {
  if (access("/usr/local/share/doc/efte/README", 0) == 0) FileLoad(
      0,
      "/usr/local/share/doc/efte/README",
      0,
      this);
  else if (access("/usr/share/doc/efte/README", 0) == 0) FileLoad(
      0,
      "/usr/share/doc/efte/README",
      0,
      this);
  else if (access("/efte/doc/README", 0) == 0) FileLoad(0,
                                                        "/efte/doc/README",
                                                        0,
                                                        this);
  else if (access("/efte/README", 0) == 0) FileLoad(0, "/efte/README", 0, this);
  else if (access("/Program Files/efte/doc/README", 0) == 0) FileLoad(
      0,
      "/Program Files/efte/doc/README",
      0,
      this);
  else if (access("/Program Files (x86)/doc/README", 0) == 0) FileLoad(
      0,
      "/Program Files (x86)/doc/README",
      0,
      this);
  else MView->Win->Choice(0, "About", 1, "O&K", PROGRAM " " VERSION " " COPYRIGHT);
  return 1;
}

int EView::ViewModeMap(ExState& /*State*/) {
  if (TheEventMapView != 0) TheEventMapView->ViewMap(GetEventMap());
  else (void)new EventMapView(0, &ActiveModel, GetEventMap());

  if (TheEventMapView != 0) SwitchToModel(TheEventMapView);
  else return 0;
...
int EView::TagLoad(ExState& State) {
  ...
  if (ExpandPath(pTagFile, Tag, sizeof(Tag)) == -1) return 0;

  if (State.GetStrParam(this, Tag,
                        sizeof(Tag)) ==
      0) if (MView->Win->GetFile("Load tags", sizeof(Tag), Tag, HIST_TAGFILES,
                                 GF_OPEN) == 0) return 0;

  if (ExpandPath(Tag, FullTag, sizeof(FullTag)) == -1) return 0;
...
int EView::ConfigRecompile(ExState& /*State*/) {
  ...
  if (ExpandPath("~/.efterc", command + strlen(command), sizeof(command) -
                 strlen(command)) != 0) return 0;
...
int EView::RemoveGlobalBookmark(ExState& State) {
  ...
  if (State.GetStrParam(this, name,
                        sizeof(name)) ==
      0) if (MView->Win->GetStr("Remove Global Bookmark", sizeof(name), name,
                                HIST_BOOKMARK) == 0) return 0;
...
int EView::GotoGlobalBookmark(ExState& State) {
  ...
  if (State.GetStrParam(this, name,
                        sizeof(name)) ==
      0) if (MView->Win->GetStr("Goto Global Bookmark", sizeof(name), name,
                                HIST_BOOKMARK) == 0) return 0;
...
int EView::Cvs(ExState& State) {
  ...
  if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
    if (MView->Win->GetStr("CVS options", sizeof(Opts), Opts,
                           HIST_CVS) == 0) return 0;

    strcpy(Options, Opts);
  } else {
    if (MView->Win->GetStr("CVS options", sizeof(Options), Options,
                           HIST_CVS) == 0) return 0;
  }
  return Cvs(Options);
}
...
int EView::Cvs(const char *Options) {
  ...
  case CONTEXT_FILE:
    ...
    if (JustFileName(((EBuffer *)Model)->FileName, OnFiles,
                     sizeof(buf)) != 0) return 0;  // OnFiles points to buf
...
int EView::CvsDiff(ExState& State) {
  ...
  if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
    if (MView->Win->GetStr("CVS diff options", sizeof(Opts), Opts,
                           HIST_CVSDIFF) == 0) return 0;

    strcpy(Options, Opts);
  } else {
    if (MView->Win->GetStr("CVS diff options", sizeof(Options), Options,
                           HIST_CVSDIFF) == 0) return 0;
  }
...
int EView::CvsCommit(ExState& State) {
  ...
  if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
    if (MView->Win->GetStr("CVS commit options", sizeof(Opts), Opts,
                           HIST_CVSCOMMIT) == 0) return 0;

    strcpy(Options, Opts);
  } else {
    if (MView->Win->GetStr("CVS commit options", sizeof(Options), Options,
                           HIST_CVSCOMMIT) == 0) return 0;
  }
...
int EView::Svn(ExState& State) {
  ...
  if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
    if (MView->Win->GetStr("SVN options", sizeof(Opts), Opts,
                           HIST_SVN) == 0) return 0;

    strcpy(Options, Opts);
  } else {
    if (MView->Win->GetStr("SVN options", sizeof(Options), Options,
                           HIST_SVN) == 0) return 0;
  }
  return Svn(Options);
}
...
int EView::SvnDiff(ExState& State) {
  ...
  if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
    if (MView->Win->GetStr("SVN diff options", sizeof(Opts), Opts,
                           HIST_SVNDIFF) == 0) return 0;

    strcpy(Options, Opts);
  } else {
    if (MView->Win->GetStr("SVN diff options", sizeof(Options), Options,
                           HIST_SVNDIFF) == 0) return 0;
  }
...
int EView::SvnCommit(ExState& State) {
  ...
  if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
    if (MView->Win->GetStr("SVN commit options", sizeof(Opts), Opts,
                           HIST_SVNCOMMIT) == 0) return 0;

    strcpy(Options, Opts);
  } else {
    if (MView->Win->GetStr("SVN commit options", sizeof(Options), Options,
                           HIST_SVNCOMMIT) == 0) return 0;
  }
