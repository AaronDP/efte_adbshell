...
static int i18n_key_analyze(XKeyEvent *keyEvent, KeySym *key, /*FOLD00*/
  ...
  /* Select keyboard map */
  if (!i) kbdActual = nationalKey[0];
  else if (!remap) kbdActual = nationalKey[1];
  else kbdActual = nationalKey[2];
  ...
  } else if (keypos) return 0;  /* ignore key release */
...
static int i18n_key_analyze(XKeyEvent * /*keyEvent*/, KeySym *key, /*FOLD00*/
   ...
   */
  if ((nbytes == 1) && (*key < KEYBYTEMAX)) *key = t;
...
# endif // ifdef USE_HACK_FOR_BAD_XSERVER
  return nbytes;
}

/*FOLD00*/
#endif // ifdef USE_HARD_REMAP
  ...
  *mask = 0;
  ...
  for (s = tmp; s && !found;) {
    ...
    *end = '\0';
    ...
    if (!strcmp(s,
                "OverTheSpot")) ctx->input_style =
        (XIMPreeditPosition | XIMStatusArea);
    else if (!strcmp(s,
                     "OffTheSpot")) ctx->input_style =
        (XIMPreeditArea | XIMStatusArea);
    else if (!strcmp(s,
                     "Root")) ctx->input_style =
        (XIMPreeditNothing | XIMStatusNothing);

    for (i = 0; (unsigned short)i < ctx->xim_styles->count_styles; i++)
      if (ctx->input_style == ctx->xim_styles->supported_styles[i])

      // if (ctx->xim_styles->supported_styles[i] & (XIMPreeditNothing |
      // XIMPreeditNone) &&
      //    ctx->xim_styles->supported_styles[i] & (XIMStatusNothing  |
      // XIMStatusNone))
      {
        found = True;
  ...
  if (ctx->xic == NULL) {
    ...
    i18n_destroy(&ctx);
  } else if (XGetICValues(ctx->xic, XNFilterEvents, mask, NULL)) fprintf(
      stderr,
      "I18N error: Can't get Event Mask\n");
  return ctx;
...
void i18n_focus_in(i18n_context_t *ctx) { /*FOLD00*/
  ...
  if (ctx->xic != NULL) XSetICFocus(ctx->xic);
#endif // if XlibSpecificationRelease >= 6
}
...
void i18n_focus_out(i18n_context_t *ctx) { /*FOLD00*/
  ...
  if (ctx->xic != NULL) XUnsetICFocus(ctx->xic);
#endif // if XlibSpecificationRelease >= 6
}
...
# endif // if 1
  ...
  if (((*key == XK_F1) || (*key == XK_F11))
      ...
      && (keyEvent->type == KeyPress)) showKeys = !showKeys;
