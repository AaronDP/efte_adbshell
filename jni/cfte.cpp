...
static int LoadFile(const char *WhereName,
                    const char *CfgName,
                    int         Level = 1,
                    int         optional = 0);
...
static void PutObject(CurPos& cp, int xtag, int xlen, void *obj) {
  ...
  if (cpos >= CACHE_SIZE) Fail(cp, "Cache exceeded");
}
...
static int Lookup(const OrdLookup *where, const char *what) {
  ...
  for (i = 0; where[i].Name != 0; i++) {
    if (stricmp(what, where[i].Name) == 0) return where[i].num;
  }
  return -1;
}
...
#define K_OINCLUDE     24 // Optional include, i.e. do not fail if it does not
                          // exist.
...
static int DefinedWord(const char *w) {
  if ((words == 0) || (wordCount == 0)) return 0;
...
static int DefineColor(char *name, char *value) {
  ...
  colors =
    (struct _color *)realloc(colors, sizeof(struct _color) * (colorCount + 1));
    assert(colors != 0);
...
static char* DefinedColor(const char *name) {
  if ((colors == 0) || (colorCount == 0)) return 0;

  for (int i = 0; i < colorCount; i++)
    if (strcmp(name, colors[i].colorName) == 0) return colors[i].colorValue;
...
static char* GetColor(CurPos& cp, char *name) {
  ...
  if ((c =
         strchr(name,
                ' ')) != NULL) {} else if ((c = strchr(name, ':')) != NULL) {
    char clr[4];
    *c++   = 0;
    ...
    if (!p) Fail(cp, "Unknown symbolic color %s", name);
    name = p;
  }
...
static int Parse(CurPos& cp) {
  while (cp.c < cp.z) {
    ...
    default:
      ...
      if (((*cp.c >= 'a') && (*cp.c <= 'z')) ||
          ((*cp.c >= 'A') && (*cp.c <= 'Z')) ||
          (*cp.c == '_')) return P_WORD;
      else return P_SYNTAX;
    }
  }
  return P_EOF;
}
...
static int CFteCmdNum(const char *Cmd) {
  ...
  for (i = 0; i < sizeof(Command_Table) / sizeof(Command_Table[0]); i++)
    if (stricmp(Cmd, Command_Table[i].Name) == 0) return Command_Table[i].CmdId;

  for (i = 0; i < CCFteMacros; i++)
    if (CFteMacros[i].Name &&
        ((stricmp(Cmd, CFteMacros[i].Name)) == 0)) return i | CMD_EXT;
...
int NewCommand(CurPos& cp, const char *Name) {
  if (Name == 0) Name = "";
  CFteMacros =
    (DefinedMacro *)realloc(CFteMacros, sizeof(DefinedMacro) * (1 + CCFteMacros));
...
static int ParseCommands(CurPos& cp, char *Name) {
  ...
  while (1) {
    ...
    if (p == P_DOT) {
      ...
      if (Command == 0) Fail(cp, "Unrecognized command: %s", cmd);
  ...
  while (1) {
    ...
    case P_WORD:
      ...
      case K_SUB: {
        ...
        if (ParseCommands(cp, strdup(Name)) == -1) Fail(cp, "Parse failed");
        PutNull(cp, CF_END);
        break;
      }
      ...
      case K_MENU: {
        ...
        while (1) {
          ...
          case K_SUBMENU: // menu::submenu
            ...
            if (GetWord(cp, w) == -1) Fail(cp, "Parse failed");
        ...
        while (1) {
          ...
          case K_COLOR: // mode::color
            ...
            while (1) {
              ...
              if ((cidx = Lookup(hilit_colors, sname)) == -1) Fail(
                  cp,
                  "Lookup of '%s' failed",
                  sname);
              ...
              if (Parse(cp) != P_COMMA) break;
              else GetOp(cp, P_COMMA);
            }
          ...
          case K_HSTATE: {
            ...
            if ((cidx = Lookup(hilit_colors, cname)) == -1) Fail(
                cp,
                "Lookup of '%s' failed",
                cname);
            PutNumber(cp, CF_INT, cidx);
          ...
          case K_HTRANS: {
            ...
            PutNumber(cp, CF_INT, cidx);
            PutString(cp,
                      match_opts & MATCH_REGEXP ? CF_REGEXP : CF_STRING,
                      match);
      ...
      case K_MODE: { // mode::
        ...
        if (Parse(cp) == P_COLON) {
          ...
          if (GetWord(cp, UpMode) != 0) Fail(
              cp,
              "Parse failed, expecting a mode name");
        }
        PutString(cp, CF_PARENT, UpMode);
      ...
      case K_COLPALETTE: {
        ...
        while (1) {
          ...
          if (DefineColor(sname, svalue) != 1) Fail(cp, "DefineColor failed\n");
      ...
      case K_INCLUDE: {
        ...
        if (verbosity > 0) fprintf(stderr, ACTION "%s... ", "include", fn);

        if (LoadFile(cp.name, fn) != 0) Fail(cp,
                                             "Include of file '%s' failed",
                                             fn);
      ...
      case K_OINCLUDE: {
        ...
        if (verbosity > 1) fprintf(stderr, ACTION "%s... ", "opt include", fn);

        if (LoadFile(cp.name, fn, 1, 1) != 0) {
          if (verbosity > 1) fprintf(stderr, "not found\n");
          GetOp(cp, P_EOS);
    ...
    case '%':
      ...
      if ((cp.c + 8 < cp.z) && (strncmp(cp.c, "%define(", 8) == 0)) {
        ...
        while (cp.c < cp.z && *cp.c != ')') {
          ...
          if (DefinedWord(w)) wasWord = 1;

          if (neg) wasWord = wasWord ? 0 : 1;
...
int ProcessConfigFile(char *filename, char *buffer, int Level) {
  ...
  if (strcmp(filename, "built-in") != 0) free(buffer);
...
# define SEARCH_PATH_LEN 11
    ...
    if ((found == false) && (optional == 1)) return -1;
    else if (found == false) {
      fprintf(stderr,
  ...
  if (verbosity) fprintf(stderr, "found: %s\n", Cfg);

  if ((fd = open(Cfg, O_RDONLY | O_BINARY)) == -1) {
    if (!optional) fprintf(stderr, "Cannot open '%s', errno=%d\n", Cfg, errno);
    return -1;
  }
  ...
  if (fstat(fd, &statbuf) != 0) {
    ...
    if (!optional) fprintf(stderr, "Cannot stat '%s', errno=%d\n", Cfg, errno);
    return -1;
  }
  ...
  buffer[statbuf.st_size] = 0; // add null to end of buffer, NOTE: allocated
                               // statbuf.st_size + 1
