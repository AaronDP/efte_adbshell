...
ExChoice::ExChoice(const char *ATitle,
                   int         NSel,
                   va_list     ap) : ExView() {
  char  msg[1024];
  int   i;
...
ExChoice::~ExChoice() {
  ...
  for (int i = 0; i < NOpt; i++) free(SOpt[i]);
}
...
void ExChoice::HandleEvent(TEvent& Event) {
  ...
  case evMouseDown:
    ...
    if (Win->CaptureMouse(1)) MouseCaptured = 1;
    else break;
  ...
  case evMouseMove:
    ...
    if (MouseCaptured) Cur = FindChoiceByPoint(Event.Mouse.X, Event.Mouse.Y);
    Event.What = evNone;
    break;

  case evMouseUp:

    if (MouseCaptured) Win->CaptureMouse(0);
    else break;
    MouseCaptured = 0;
    Cur           = FindChoiceByPoint(Event.Mouse.X, Event.Mouse.Y);
    Event.What    = evNone;

    if ((Cur >= 0) && (Cur < NOpt) && (NOpt > 0)) EndExec(Cur);
    else Cur = 0;
    break;
  }
}
...
void ExChoice::RepaintStatus() {
  ...
  for (i = 0; i < NOpt; i++) {
    ...
    if (i == Cur) ConSetCursorPos(pos + 1, H - 1);
