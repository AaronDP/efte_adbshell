...
void EListPort::HandleMouse(TEvent& Event) {
  ...
  case evMouseDown:
    ...
    if (Event.Mouse.Buttons == 2)
      if ((yy < List->Count) && (yy >= 0)) List->SetPos(yy, LeftCol);
    Event.What = evNone;
    break;
...
void EListPort::UpdateView() {
  if ((OldLeftCol != LeftCol) || (OldTopRow != TopRow) ||
      (OldCount != List->Count)) List->NeedsRedraw = List->NeedsUpdate = 1;
...
void EListPort::PaintView(int PaintAll) {
  ...
  if (List->NeedsRedraw) PaintAll = 1;
  ...
  if (View->MView->Win->GetViewContext() != View->MView) return;
  ...
  for (I = 0; I < H; I++) {
    if (PaintAll || (I + TopRow == Row) || (I + TopRow == OldRow)) {
      ...
      if (I + TopRow < List->Count) List->DrawLine(B,
                                                   I + TopRow,
                                                   LeftCol,
                                                   color,
                                                   W);
...
void EListPort::RepaintStatus() {
  ...
  if (View->MView->Win->GetStatusContext() != View->MView) return;
  ...
  if (View->CurMsg == 0) {
    if (List->Title) MoveStr(B, 0, W, List->Title, SColor, W);
  ...
  if ((View->MView->Win->GetStatusContext() == View->MView) &&
      View->MView->Win->IsActive()) View->MView->Win->ConSetCursorPos(
      0,
      Row -
      TopRow);
}

EList::EList(int         createFlags,
             EModel    **ARoot,
...
void EList::SetTitle(const char *ATitle) {
  ...
  if (View && View->MView) View->MView->RepaintStatus();
}
...
void EList::FixPos() {
  ...
  if (!WeirdScroll) if (TopRow + H > Count) TopRow = Count - H;
  ...
  if (TopRow + scrollBorderY > Row) TopRow = Row - scrollJumpY + 1 -
                                             scrollBorderY;

  if (TopRow + H - scrollBorderY <= Row) TopRow = Row - H + 1 + scrollJumpY - 1 +
                                                  scrollBorderY;
...
int EList::ScrollUp(int Rows) {
  if (TopRow == Count - 1) return ErFAIL;
...
int EList::MovePageEnd() {
  ...
  if (Row == Count - 1) return ErOK;

  View->MView->Win->ConQuerySize(&W, &H);
  H--;

  if (Row == TopRow + H - 1) return ErOK;
...
int EList::MoveFileStart() {
  if ((Row == 0) && (LeftCol == 0)) return ErOK;
...
int EList::MoveFileEnd() {
  if ((Row == Count - 1) && (LeftCol == 0)) return 0;
...
int EList::MarkAll() {
  ...
  for (int i = 0; i < Count; i++) {
    if (!IsMarked(i)) if (Mark(i) != 1) return ErFAIL;
  }
  return ErOK;
}
...
int EList::UnmarkAll() {
  ...
  for (int i = 0; i < Count; i++) {
    if (IsMarked(i)) if (Unmark(i) != 1) return ErFAIL;
  }
  return ErOK;
}
