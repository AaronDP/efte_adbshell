...
extern int ExecMainMenu(TEvent& E,
                        char    sub);
extern int ExecVertMenu(int     x,
                        int     y,
                        int     id,
                        TEvent& E,
                        UpMenu *up);
...
GViewPeer::~GViewPeer() {
  if (MouseCapture == View) MouseCapture = 0;

  if (FocusCapture == View) FocusCapture = 0;
}
...
int GViewPeer::ConSetCursorPos(int X, int Y) {
  ...
  if (wState & sfFocus) return ::ConSetCursorPos(cX + wX, cY + wY);
  else return 1;
}
...
int GViewPeer::ConShowCursor() {
  ...
  if (wState & sfFocus) return ::ConShowCursor();
  else return 1;
}
...
int GViewPeer::ConHideCursor() {
  ...
  if (wState & sfFocus) return ::ConHideCursor();
  else return 1;
}
...
int GViewPeer::CaptureMouse(int grab) {
  if (MouseCapture == 0) {
    ...
    if (grab || (MouseCapture != View)) return 0;
    else MouseCapture = 0;
  }
  return 1;
}
...
int GViewPeer::CaptureFocus(int grab) {
  if (FocusCapture == 0) {
    ...
    if (grab || (FocusCapture != View)) return 0;
    else FocusCapture = 0;
  }
  return 1;
}
...
int GViewPeer::ExpandHeight(int DeltaY) {
  ...
  if (DeltaY + wH < 3) DeltaY = -(wH - 3);

  if (View->Next->Peer->wH - DeltaY < 3) DeltaY = View->Next->Peer->wH - 3;
...
int GViewPeer::DrawScrollBar() {
  ...
  if (ShowVScroll) {
    ...
    for (I = 0; I < NRows; I++) {
      if ((I >= SbVBegin) && (I <= SbVEnd)) MoveCh(B, fore, hcScrollBar_Fore, 1);
      else MoveCh(B, back, hcScrollBar_Back, 1);
  ...
  if (ShowHScroll) {
    ...
    for (I = 0; I < NCols; I++) {
      if ((I >= SbHBegin) && (I <= SbHEnd)) MoveCh(B, fore, hcScrollBar_Fore, 1);
      else MoveCh(B, back, hcScrollBar_Back, 1);
...
int GFrame::ConSplitView(GView *view, GView *newview) {
  ...
  if (ShowVScroll) newview->Peer->wW--;
...
void GFrame::Update() {
  ...
  while (v) {
    ...
    if ((ShowVScroll ||
         ShowHScroll) && (v->Peer->sbVupdate || v->Peer->sbHupdate)) {
...
void GFrame::Resize(int width, int height) {
  ...
  if (ShowVScroll) width--;

  if (ShowHScroll) height--;
  ...
  if (y == wY) {
    do {
      ...
      do {
        ...
        if (E.What & evNotify) gui->DispatchEvent(frames, view, E);
    ...
    do {
      ...
      E1.Msg.Param1  =
        (E.Mouse.Y - wY - 1 - delta + 1) * view->Peer->sbVtotal / (wH - 2);
...
static void HandleHScroll(GView *view, TEvent& E) {
  ...
  if (x == wX) {
    ...
    do {
      ...
      E1.Msg.Param1  =
        (E.Mouse.X - wX - 1 - delta + 1) * view->Peer->sbHtotal / (wW - 2);
  ...
  if (E.What != evNone) {
    ...
    if (E.What & evMouse) {
      ...
      if (ShowVScroll && ShowHScroll && (E.What == evMouseDown) &&
          ...
          (E.Mouse.X == view->Peer->wX + view->Peer->wW)) {} else {
    ...
    if (E.What != evNone) DispatchEvent(frames, view, E);
  }
}
