...
#define UGETC(rc, no, pos, what)                                          \
  do { void *d = 0;                                                       \
       if ((rc =                                                          \
              GetUData(no, pos, &d,                                       \
                       sizeof(unsigned char)))) *(unsigned char *)&what = \
           *(unsigned char *)d;                                           \
       pos -= sizeof(unsigned char); } while (0)

// TODO: void *d is dangerous, this needs fixed!
#define UGET(rc, no, pos, what)                                               \
  do { void *d = 0;                                                           \
       if ((rc = GetUData(no, pos, &d, sizeof(what)))) memcpy((void *)&what,  \
                                                              d,              \
                                                              sizeof(what));  \
       pos -= sizeof(what); } while (0)
  ...
  while (rc == 1) {
    ...
    case ucPlaceUserBookmark:
      ...
      if ((Col == (unsigned long)-1) || (Line == (unsigned long)-1)) {
        ...
        if (PlaceUserBookmark((const char *)data,
                              EPoint(Line, Col)) == 0) return 0;
      }
      break;
