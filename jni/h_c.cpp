      ...
      case hsC_Normal:
        ...
        if ((toupper(*p) == 'L') && (p[1] == '"')) {
          ...
          if (BF->GetHilitWord(j, &Line->Chars[i], Color)) {
            ...
            while (x < Line->Count && isspace(Line->Chars[x])) x++;
...
// only numbers longer than 1 digit be hex/octal
      ...
      case hsC_CPP:
        ...
        if (ISNAME(*p)) {
          ...
          if ((j == 7) &&
              (memcmp(Line->Chars + i, "include", 7) == 0)) was_include = 1;
...
hilit:
  ...
  if (State == hsC_CommentL) State = hsC_Normal;
...
int IsState(hsState *Buf, hsState State, int Len) {
  ...
  for (I = 0; I < Len; I++) if (Buf[I] != State) return 0;

  return 1;
}

int LookAt(EBuffer     *B,
           int          Row,
           unsigned int Pos,
           const char  *What,
           hsState      State,
...
int EBuffer::SetCIndentStyle(ExState& State) {
  if (State.GetIntParam(View, &C_Indent) == 0) return 0;

  if (State.GetIntParam(View, &C_BraceOfs) == 0) return 0;
  ...
  if (State.GetIntParam(View, &C_ClassOfs) == 0) return 0;
...
static int CheckLabel(EBuffer *B, int Line) {
  ...
  if ((Line > 0) && (B->RLine(Line - 1)->StateE != hsC_Normal)) return 0;
...
static int SearchBackMatch(int         Count,
                           EBuffer    *B,
                           int         Row,
                           hsState     State,
                           const char *Open,
...
static int FindPrevIndent(EBuffer *B,
                          int    & RowP,
                          int    & ColP,
                          char   & CharP,
                          int      Flags) {
...
static int IndentNormal(EBuffer *B, int Line, int /*StateLen*/,
  ...
  if (LookAt(B, Line, 0, "case", hsC_Keyword) ||
    ...
    I =
      SearchBackMatch(-1, B, Line - 1, hsC_Normal, "{", "}", &Pos,
                      &L) + C_CASE_OFS;
    return I;
    ...
    if (C_PAREN_DELTA >= 0) return I + C_PAREN_DELTA;
    else return Pos;
  } else if (LookAt(B, Line, 0, "]", hsC_Normal, 0)) {
    I = SearchBackMatch(-1, B, Line - 1, hsC_Normal, "[", "]", &Pos, &L);
    ...
    if (SkipWhite(B, Line, PrevRowP, PrevColP, SKIP_BACK) != 1) return 0;
...
// FIND_CLASS |
                       ...
                       FIND_COMMA |
                       FIND_ENDBLOCK) != 1) {
      if (RowP != PrevRowP) ContinuationIndent = C_CONTINUATION;
      I = 0;
    ...
    case '[':
      ...
      if (C_PAREN_DELTA >= 0) {
        ...
        if (SkipWhite(B, Line, RowP, ColP,
                      SKIP_FORWARD | SKIP_LINE) != 1) return 0;
    ...
    case '{':
      ...
      if (((PrevRowP != RowP) ||
          ...
          && (FirstRowP != PrevRowP)) ContinuationIndent = C_CONTINUATION;
      ColP--;
      ColP--;

      if (SkipWhite(B, Line, RowP, ColP,
                    SKIP_BACK | SKIP_TOBOL | SKIP_MATCH) != 1) return 0;

      I = B->LineIndented(RowP);

      if (B->LineIndented(FirstRowP) <= C_FIRST_WIDTH) I += C_FIRST_INDENT;
      else I += C_INDENT;
        PRINTF(("'{' indent : Line=%d, RowP=%d, ColP=%d, CharP=%c\n", Line, RowP,
              ColP, CharP));

      if (LookAt(B, Line, 0, "{", hsC_Normal, 0)) I -= C_BRACE_OFS;
      else I += ContinuationIndent;
...
// (CharP == ';') ? FIND_SEMICOLON | FIND_COLON :
        ...
        if (FirstRowP != PrevRowP) ContinuationIndent = C_CONTINUATION;
        I = 0;
      ...
      case ':':
        ...
        if (SkipWhite(B, Line, RowP, ColP, SKIP_FORWARD) != 1) return 0;
        ...
        if (LookAt(B, Line, 0, "{", hsC_Normal, 0)
            && LookAt(B, RowP, ColP, "{", hsC_Normal, 0)) I -= 0;  // C_BRACE_OFS;
        else if (LookAt(B, Line, 0, "{", hsC_Normal, 0)
                 && !LookAt(B, RowP, ColP, "{", hsC_Normal, 0)) I += C_BRACE_OFS;
        else if (!LookAt(B, Line, 0, "{", hsC_Normal, 0)
                 && LookAt(B, RowP, ColP, "{", hsC_Normal, 0)) I -= C_BRACE_OFS;
        break;
    ...
    case ':':
      ...
      case ';':
        ...
        if (CheckLabel(B, RowP)) I -= C_COLON_OFS;
        else if ((PrevRowP == RowP) && (FirstRowP == PrevRowP) &&
                 (FirstColP + 1 == PrevColP)) I += C_CONTINUATION;
    ...
    case 'w':
      ...
      if (SkipWhite(B, Line, RowP, ColP,
                    SKIP_FORWARD |
                    ((CharP != 'e') ? SKIP_MATCH : 0)) != 1) return 0;
      ...
      if (SkipWhite(B, Line, RowP, ColP, SKIP_BACK) != 1) return 0;
...
static int IndentComment(EBuffer *B, int Row, int /*StateLen*/,
  ...
  if (Row > 0) {
    ...
    if (B->RLine(Row - 1)->StateE ==
        hsC_Comment) if (LookAt(B, Row - 1, I, "/*", hsC_Comment,
                                0)) I += C_COMMENT_DELTA;

    if (B->RLine(Row - 1)->StateE ==
        hsC_CPP_Comm) if (LookAt(B, Row - 1, I, "/*", hsC_CPP_Comm,
                                 0)) I += C_COMMENT_DELTA;
  }
  return I;
}
...
static int IndentCPP(EBuffer *B, int Line, int /*StateLen*/,
                     hsState * /*StateMap*/) {
  if (LookAt(B, Line, 0, "#", hsC_CPP, 0)) return 0;
  else return C_INDENT;
}
