...
ExInput::ExInput(const char  *APrompt,
                 char        *ALine,
  ...
  case evKeyDown:
    ...
    case kbLeft | kfCtrl:
      ...
      if (Pos > 0) {
        ...
        while (Pos > 0) {
          if (isalnum(Line[Pos]) && !isalnum(Line[Pos - 1])) break;
          Pos--;
        }
      }
      SelStart   = SelEnd = 0;
    ...
    case kbBackSp | kfShift:
      ...
      if (Pos <
          strlen(Line)) memmove(Line + Pos, Line + Pos + 1, strlen(
                                  Line + Pos + 1) + 1);
      TabCount   = 0;
      Event.What = evNone;
      break;
    ...
    case kbUp:
      ...
      if (CurItem == 0) strcpy(CurStr, Line);
      CurItem += 2;
    ...
    case kbDown:
      ...
      CurItem--;
      {
        ...
        if (CurItem == 0) strcpy(Line, CurStr);
...
void ExInput::RepaintStatus() {
  ...
  if (Pos > strlen(Line)) Pos = strlen(Line);
