...
ELine::ELine(int         ACount,
  ...
  if (AChars) memcpy(Chars, AChars, Count);
  else memset(Chars, ' ', Count);
}

ELine::ELine(char *AChars,
             int   ACount) {
  Chars              = AChars;
  Count              = ACount;
  StateE             = 0;
...
int ELine::Allocate(unsigned int Bytes) {
  ...
  if (Chars) {
    ...
    if (ReallocChars == NULL) free(Chars);
    Chars = ReallocChars;
  } else {
...
int EBuffer::RToVN(int No) {
  ...
  if (No == RCount) return VCount;

  if (No > Vis(VCount - 1) + VCount - 1) return VCount - 1;

  if (No < VCount) if (Vis(No) == 0) return No;
  ...
  while (L < R) {
    ...
    if (V == No) return M;
      ...
      if (M == VCount - 1) return M;
      else if (Vis(M + 1) + M + 1 > No) return M;
