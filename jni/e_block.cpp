...
int EBuffer::SetBB(EPoint M) {
  ...
  if (MinL != -1) if (MinL <= MaxL) Draw(MinL, MaxL);
  return 1;
}
...
int EBuffer::BlockCopy(int Append, int clipboard) {
  ...
  if (Append) {
    if (SystemClipboard) GetPMClip(clipboard);
  } else SSBuffer->Clear();
  ...
  case bmColumn:
    ...
    for (L = B.Row; L < E.Row; L++) {
      ...
      if (SSBuffer->InsLineText(SL, 0, E.Col - B.Col, B.Col,
                                RLine(L)) == 0) return 0;
  ...
  case bmStream:
    ...
    if (Append && (OldCount > 0))
      if (SSBuffer->JoinLine(OldCount - 1, 0) == 0) return 0;

    break;
  }

  if (SystemClipboard) PutPMClip(clipboard);
  return 1;
}
...
int EBuffer::BlockPaste(int clipboard) {
  ...
  case bmLine:
    ...
    for (L = 0; L < SSBuffer->RCount; L++) {
      ...
      if (InsLineText(BL, 0, SSBuffer->LineLen(L), 0,
                      SSBuffer->RLine(L)) == 0) return 0;
  ...
  case bmColumn:
    ...
    for (L = 0; L < SSBuffer->RCount; L++) {
      ...
      if (InsLineText(BL, B.Col, SSBuffer->LineLen(L), 0,
                      SSBuffer->RLine(L)) == 0) return 0;
  ...
  case bmStream:
    ...
    if (SSBuffer->RCount > 1) if (SplitLine(B.Row, B.Col) == 0) return 0;

    if (InsLineText(B.Row, B.Col, SSBuffer->LineLen(0), 0,
                    SSBuffer->RLine(0)) == 0) return 0;
...
int EBuffer::BlockKill() {
  ...
  case bmColumn:
    ...
    for (L = B.Row; L < E.Row;
         L++) if (DelText(L, B.Col, E.Col - B.Col) == 0) return 0;
...
int EBuffer::BlockExtendBegin() {
  ...
  if (ExtendGrab == 0) {
    ...
    if (BlockMode == bmColumn) ExtendGrab = 1 | 2 | 4 | 8;
    else ExtendGrab = 1 | 2;
  }
  return 1;
}
...
static int _LNK_CONV SortProc(const void *A, const void *B) {
  ...
  if (SortMinCol == -1) {
    ...
    if (BFI(SortBuffer,
            BFI_MatchCase) == 1) rc = memcmp(LA->Chars,
                                             LB->Chars,
                                             (lA < lB) ? lA : lB);
    else rc = memicmp(LA->Chars, LB->Chars, (lA < lB) ? lA : lB);
    ...
    if ((lA < 0) && (lB < 0)) rc = 0;
    else if ((lA < 0) && (lB > 0)) rc = -1;
    else if ((lA > 0) && (lB < 0)) rc = 1;
    else {
      if (SortMaxCol != -1) {
        if (lA > SortMaxCol - SortMinCol) lA = SortMaxCol - SortMinCol;

        if (lB > SortMaxCol - SortMinCol) lB = SortMaxCol - SortMinCol;
      }

      if (BFI(SortBuffer, BFI_MatchCase) == 1) rc = memcmp(LA->Chars + PA,
                                                           LB->Chars + PB,
                                                           (lA < lB) ? lA : lB);
      else rc = memicmp(LA->Chars + PA, LB->Chars + PB, (lA < lB) ? lA : lB);
...
int EBuffer::BlockSort(int Reverse) {
  ...
  if ((BlockMode != bmStream) || (BE.Col == 0)) SortMaxRow--;

  if (SortMinRow >= SortMaxRow) return 1;
  ...
  for (rq = 0; rq <= SortMaxRow - SortMinRow; rq++) SortRows[rq] = rq +
                                                                   SortMinRow;
  ...
  for (rq = 0; rq <= SortMaxRow - SortMinRow; rq++) {
    ...
    if (InsLine(1 + rq + SortMaxRow, 0) == 0) return 0;

    if (InsChars(1 + rq + SortMaxRow, 0, oldL->Count, oldL->Chars) == 0) return 0;
  }

  for (rq = 0; rq <= SortMaxRow - SortMinRow; rq++)
    if (DelLine(SortMinRow) == 0) return 0;
  ...
  for (int i = B.Row; i < E.Row; i++) {
    ...
    while (O < L->Count) {
      if (L->Chars[O] == '\t') {
        ...
        if (DelChars(i, O, 1) != 1) return 0;

        if (InsChars(i, O, C - O, 0) != 1) return 0;
  ...
  for (int i = B.Row; i < E.Row; i++) {
    ...
    while (O < L->Count) {
      if (L->Chars[O] == '\t') { // see if there are spaces to remove
        ...
        if (N > 0) {
          if (DelChars(i, O - N, N) != 1) return 0;
        ...
        O1 = O;
                   ...
                   BFI(this, BFI_TabSize)) == (BFI(this, BFI_TabSize) - 1)) &&
        ...
        if (DelChars(i, O1, N) != 1) return 0;

        if (InsChars(i, O1, 1, &tab) != 1) return 0;
...
int EBuffer::FindFunction(int delta, int way) {
  ...
  while (line >= 0 && line < RCount) {
    ...
    if (RxExec(regx, L->Chars, L->Count, L->Chars, &res) == 1) break;
    line += way;
  }

  if (line < 0) line = 0;

  if (line >= RCount) line = RCount - 1;
  RxFree(regx);
  return line;
}
...
int EBuffer::BlockMarkFunction() {
  ...
  if ((by = FindFunction(0, -1)) == -1) return 0;

  if ((ey = FindFunction(+1, +1)) == -1) return 0;

  // ** Start and end are known. Set the block;
  BlockMode = bmStream;

  if (SetBB(EPoint(by, 0)) == 0) return 0;
