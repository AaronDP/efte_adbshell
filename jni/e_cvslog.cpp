...
ECvsLog::ECvsLog(int         createFlags,
                 EModel    **ARoot,
...
int ECvsLog::ConfQuit(GxView *V, int /*multiFile*/) {
  ...
  switch (V->Choice(GPC_ERROR, "CVS commit pending", 3, "C&ommit", "&Discard",
                    "&Cancel", "")) {
  case 0: // Commit
...
void ECvsLog::GetTitle(char *ATitle, int MaxLen, char *ASTitle,
                       int SMaxLen) const {
