...
int TagsLoad(int id) { /*FOLD00*/
  ...
  if ((fd = open(TagMem + TagFiles[id], O_BINARY | O_RDONLY)) == -1) return -1;
  ...
  while (p < e) {
    ...
    if (LLine[0] == '/') {
      ...
      *d = 0;
      ...
      if (stricmp(d - 10, "/*FOLD00*/") == 0) d[-11] = 0;  /* remove our
                                                             internal folds */
      ...
      if (AddTag(MTag, MFile, TagFiles[id], -1, MStr) == -1) break;
    } else {
      if (AddTag(MTag, MFile, TagFiles[id], atoi(LLine), -1) == -1) break;
    }
  }
    free(tags);
  return 0;
...
int TagLoad(const char *FileName) { /*FOLD00*/
  if (TagsAdd(FileName) == 0) return 0;
...
static int GotoFilePos(EView *View, const char *FileName, int Line, int Col) { /*FOLD00*/
  if (FileLoad(0, FileName, 0, View) == 0) return 0;

  if (((EBuffer *)ActiveModel)->Loaded == 0) ((EBuffer *)ActiveModel)->Load();
...
static int GotoTag(int M, EView *View) { /*FOLD00*/
  ...
  if (TT->Line != -1) {
    if (GotoFilePos(View, path, TT->Line - 1, 0) == 0) return 0;
  } else {
    if (GotoFilePos(View, path, 0, 0) == 0) return 0;

    if (((EBuffer *)ActiveModel)->FindStr(TagMem + TT->StrFind,
                                          strlen(TagMem + TT->StrFind),
                                          0) == 0) return 0;
  }
...
int TagGoto(EView *View, char *Tag) {
  ...
  while (L < R) {
    ...
    if (cmp == 0) {
      while (M > 0 && strcmp(Tag, TagMem + TagD[TagI[M - 1]].Tag) == 0) M--;
...
int TagFind(EBuffer *B, EView *View, const char *Tag) { /*FOLD00*/
  ...
  if (CurrentTag) {
    if (strcmp(CurrentTag, Tag) == 0) {
      if (PushPos(B) == 0) return 0;

      free(CurrentTag);
      CurrentTag = strdup(Tag);

      if (CurrentTag == 0) return 0;
...
int TagComplete(char **Words, int *WordsPos, int WordsMax, const char *Tag) {
  if ((Tag == NULL) || (Words == NULL) || (*WordsPos >= WordsMax)) return 0;
  ...
  while (L < R) {
    ...
    if (c == 0) {
      while (M > 0 &&
             strncmp(Tag, TagMem + TagD[TagI[M - 1]].Tag, len) == 0) M--;  //
                                                                           // find
                                                                           // begining
      int N = M, w = 0;
        ...
        if (!w || strcmp(TagMem + TagD[TagI[N]].Tag,
          ...
          if (l > 0) {
            ...
            if (s != NULL) {
              ...
              if (*WordsPos >= WordsMax) break;
            } else break;  // how about using exceptions
          }
        }
        N++;
      }
...
int TagNext(EView *View) { /*FOLD00*/
  ...
  if ((TagPosition < CTags - 1) &&
      (strcmp(CurrentTag, TagMem + TagD[TagI[TagPosition + 1]].Tag) == 0)) {
    TagPosition++;

    if (GotoTag(TagPosition, View) == 0) return 0;
