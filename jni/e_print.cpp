...
int EBuffer::BlockPrint() {
...
#endif // if !defined(__IBMCPP__) && !defined(__WATCOMC__)
  ...
  for (L = B.Row; L <= E.Row; L++) {
    ...
    if ((A != -1) && (Z != -1)) {
      ...
      if ((lc % 200) == 0) Msg(S_INFO, "Printing, %d lines, %d bytes.", lc, bc);
    }
  }
