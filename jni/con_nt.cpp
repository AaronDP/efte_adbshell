 ...
 *              Removed most mouse handling code (unnecessary), added pipe
 *handler by
 *              porting the OS/2 version to NT.
 *              Solved some bugs with regard to the TCell problem.
 *
 *      10/28/96 Jal:
 *              Started to replace threaded pipe code with nonthreaded code,
 *using
 *              overlapped I/O.
 *
 *      Wed Jan 15 1997 (Jal):
 *              -       The Grey-Alt-+ key and some other keys were not
 *recognised. This
 *                      was because NT didn't mark these as "enhanced" keys. Now
 *the code
 *                      translates some (scancode,ascii) pairs to the enhanced
 *keyboard.
 *                      The table was already present!
 *              -       To solve the "flashing cursor" problem: now doesn't
 *enter FTE
...
static void DrawCursor(int Show) {
  if ((cci.bVisible == CursorVisible) &&
      (((cci.dwSize == 100) &&
        CursorInsert) || ((cci.dwSize == 10) && (CursorInsert == 0)))) return;
...
static struct {
  ...
  { 0x1C00,
    kbEnter          },                              { 0xE00D, kbEnter | kfGray },
  { 0xA600,
    kbEnter | kfGray },                              { 0xE00A, kbEnter | kfGray },
  ...
  { 0x0F00,
    kbTab            },                              { 0x4E00, '+' | kfGray     },
  { 0x9000,
    '+' | kfGray     },                              { 0x4E2B, '+' | kfGray     },
  { 0x4A00,
    '-' | kfGray     },                              { 0x8E00, '-' | kfGray     },
  { 0x4A2D,
    '-' | kfGray     },                              { 0x3700, '*' | kfGray     },
  { 0x9600,
    '*' | kfGray     },                              { 0x372A, '*' | kfGray     },
  { 0xE02F,
    '/' | kfGray     },                              { 0xA400, '/' | kfGray     },
  { 0x9500,
    '/' | kfGray     },                              { 0x0300,                0 }
};
#endif // if 1
...
static struct {
  ...
  { 0x79,
   '2'                  },
  { 0x7A, '3'              }, { 0x7B, '4'  }, { 0x7C, '5' },
  { 0x7D,
    '6'              },
  { 0x7E,
   '7'                  },
  { 0x7F, '8'              }, { 0x80, '9'  }, { 0x81, '0' },
  ...
  { 0x71,
    kbF10            },
  { 0x8B, kbF11                },      { 0x8C, kbF12            },

  { 0x47,
    kbHome           },
  { 0x48, kbUp                 },      { 0x49, kbPgUp           },
  { 0x4B,
    kbLeft           },
  { 0x4C, kbCenter             },      { 0x4D, kbRight          },
  { 0x4F,
    kbEnd            },
  { 0x50, kbDown               },      { 0x51, kbPgDn           },
  { 0x52,
    kbIns            },
  { 0x53, kbDel                },
  ...
  { 0x77,
    kbHome           },
  { 0x8D, kbUp                 },      { 0x84, kbPgUp           },
  { 0x73,
    kbLeft           },
  { 0x74, kbRight              },
  { 0x75,
    kbEnd            },
  { 0x91, kbDown               },      { 0x76, kbPgDn           },
  { 0x92,
    kbIns            },
  { 0x93, kbDel                },
  ...
  { 0x97,
    kbHome  | kfGray },
  { 0x98, kbUp        | kfGray },      { 0x99, kbPgUp  | kfGray },
  { 0x9B,
    kbLeft  | kfGray },
  { 0x9D, kbRight | kfGray     },
  { 0x9F,
    kbEnd   | kfGray },
  { 0xA0, kbDown  | kfGray     },      { 0xA1, kbPgDn  | kfGray },
  { 0xA2,
    kbIns   | kfGray },
  { 0xA3, kbDel       | kfGray }
};
...
int ReadConsoleEvent(TEvent *E) {
  /*
   *      Reads and interprets the console event. It is called when console
   *input
  ...
  case KEY_EVENT:
    ...
    if (Ch == 0) {
      unsigned int cc =
        ((inp.Event.KeyEvent.wVirtualScanCode <<
          8) | (unsigned char)inp.Event.KeyEvent.uChar.AsciiChar);
    ...
    if (Ch == 0) {
      if ((Ch = (TKeyCode)(unsigned char)inp.Event.KeyEvent.uChar.AsciiChar) !=
          0) {
  ...
  case MOUSE_EVENT:
    ...
    if (inp.Event.MouseEvent.dwEventFlags & DOUBLE_CLICK) E->Mouse.Count = 2;
    ...
    if (inp.Event.MouseEvent.dwEventFlags == MOUSE_MOVED) {
      ...
      if (inp.Event.MouseEvent.dwButtonState & ~mb) {
        ...
        E->What          = evMouseUp;
        E->Mouse.Buttons = mb &
                           ~((unsigned short)inp.Event.MouseEvent.dwButtonState);
...
int ConInit(int /*XSize*/, int /*YSize*/) {
  ...
  // cursor totally disappears making it hard to know where you are on the
  // screen.
...
int SaveScreen() {
  if (SavedScreen) free(SavedScreen);

  ConQuerySize(&SavedX, &SavedY);

  SavedScreen = (PCell)malloc(SavedX * SavedY * sizeof(TCell));

  if (SavedScreen) ConGetBox(0, 0, SavedX, SavedY, SavedScreen);
...
/*
 *      getFreePipe() locates an unused GPipe structure. It also assigns it's
 *ID.
 */
...
# if 1
  ...
  if (CreatePipe(&p_pipe_ph, &p_child_ph, &sa, 0) == FALSE) return FALSE;
...
# else // if 1
  ...
  if (p_pipe_ph == INVALID_HANDLE_VALUE) return FALSE;
...
#  endif // if 1
  ...
  if (p_child_ph == INVALID_HANDLE_VALUE) dbm(
      "CreateFile(client_side_pipe) has failed");
  else {
    // ** Server side: aquire connection..
    ok = TRUE;

    if (!ConnectNamedPipe(p_pipe_ph, 0)) { // Get connect;
      if (GetLastError() != ERROR_PIPE_CONNECTED) ok = FALSE;
    }

    // ** Connect worked?
    if (!ok) dbm("ConnectNmPipe() has failed");
    else return TRUE;               // All opened & ready for action!
...
int GPipe::runCommand() {
  /*
   *      runCommand() takes the child pipe, dups it onto stdout and stderr
   *while
  ...
  if (args == 0) dbm("malloc() failed for command line..");
  else {
    ...
    } else dbm("DupHandle for stderr failed.");
...
int GPipe::startRead() {
  ...
  if (p_io_posted || p_completed) dbg("no action: %s] ",
                                      p_io_posted ? "posted" : "complete");
  else {
...
int GPipe::open(char *command, EModel *notify) {
  ...
  if ((p_command = strdup(command)) == 0) return -1;
  ...
  if ((p_buffer = (char *)malloc(p_buflen)) != 0) {
    ...
    if ((pipeMutex = CreateMutex(NULL, FALSE, NULL)) == NULL) {
      ...
      if (createPipe()) { // Create server & client pipe.
        if (!postRead()) dbm("postRead() initial failed.");
        else {
          if (runCommand()) {
            p_used = TRUE;
            return p_id;
...
int GPipe::close() {
  /*
   *      close() disconnects from the spawned task, closes the pipe and
   *releases
   *      all stuff.
   */
  if (!p_used) return -1;
 ...
 *      to used. Then an event semaphore (reset_manual) is created. This
 *semaphore
 ...
 *
 *      If the spawn succeeds an overlapped READ is posted for the pipe; then
 *the
 ...
 *      overlapped result is gotten, and the output is sent to the message
 *window.
...
int GUI::OpenPipe(char *Command, EModel *notify) {
  ...
  if ((gp = GPipe::getFreePipe()) == 0) return -1;  // Out of pipes.
...
# endif // if 1
      ...
      if (CreateProcess(NULL, args, NULL, NULL, TRUE, 0, NULL, NULL, &si,
                        &pi) == TRUE) {
        ok = TRUE;
        CloseHandle(pi.hThread); // Thread handle not needed
