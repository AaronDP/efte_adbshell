...
int EBuffer::Reload() {
  ...
  if (LoadFrom(FileName) == 0) return 0;
...
int EBuffer::Save() {
  ...
  if (BFI(this, BFI_TrimOnSave)) FileTrim();
...
int EBuffer::LoadFrom(const char *AFileName) {
  ...
  while ((len = read(fd, FileBuffer, sizeof(FileBuffer))) > 0) {
    ...
    do {
      ...
      if (lf) {
        ...
        if ((lm == 0) && (m == NULL) &&
            ((m = (char *)malloc(CHAR_TRESHOLD)) == 0)) goto fail;
#if 0
        { // support for VIM tabsize commands
          char *t  = strstr(m, "vi:ts=");
          int   ts = 0;

          if (t && isdigit(t[6])) ts = atoi(&t[6]);

          if ((ts > 0) && (ts <= 16)) BFI(this, BFI_TabSize) = ts;
        }
#endif // if 0

        // Grow the line table if required,
        if (RCount == RAllocated)
          if (Allocate(RCount ? (RCount * 2) : 1) == 0) goto fail;

        if ((LL[RCount++] = new ELine((char *)m, lm)) == 0) goto fail;
        RGap = RCount;
  ...
  if ((RCount == 0) || (lm > 0) || !BFI(this, BFI_ForceNewLine)) {
    ...
    if ((LL[RCount++] = new ELine(m, lm)) == 0) goto fail;
    m    = NULL;
    RGap = RCount;
  }
  ...
  if (AllocVis(VCount ? VCount : 1) == 0) goto fail;
  ...
  if (strip != -1) { // strip CR character from EOL if specified
    ...
    for (int l = 0; l < RCount; l++) {
      if (LL[l]->Count > 0)
        if (LL[l]->Chars[LL[l]->Count - 1] == strip) LL[l]->Count--;
    }
  }
  ...
  if ((BFI(this,
    ...
    //
    // After comment start there can be a fold - open or closed - with level
    // number (nn - decimal).
    // Then bookmarks can follow marked by BOOK "keyword". Last bookmark is
    // terminated by 'x'
    // character followed by total length of bookmark info (llll - hex) and
    // character 'b'. The
    // total length is needed when looking for comment start from the line end.
    // Each bookmark has format "ccccllname" - cccc (hex) is bookmark column, ll
    // (hex) is length
    // of bookmark name and then the name itself follows.
    //
    // Example: FOLD01BOOK000a05mark1001005mark2x001ab
    //
    // Open first level fold, bookmark "mark1" at column 10 and bookmark "mark2"
    // at column 16.
    ...
    for (l = RCount - 1; l >= 0; l--) {
      if (LL[l]->Count >= len_start + len_end + 6) {
        for (int where = 1; where < 3; where++) {
          ...
          if ((BFI(this,
                   BFI_SaveFolds) != where) &&
              (BFI(this, BFI_SaveBookmarks) != where)) continue;
          ...
          if (where == 1) {
            ...
            if ((len_end != 0) &&
                (memcmp(LL[l]->Chars + pos,
                        BFS(this, BFS_CommentEnd), len_end) != 0)) continue;

            // Locate position of comment start - skip bookmarks and fold
            if ((BFI(this,
                     BFI_SaveBookmarks) == 2) && (pos - 10 >= 0) &&
                (LL[l]->Chars[pos - 1] == 'b')) {
              char numbuf[5];
              unsigned int i;
          ...
          if ((pos < 0) ||
              ((len_start != 0) &&
               (memcmp(LL[l]->Chars + pos,
                       BFS(this, BFS_CommentStart), len_start) != 0))) continue;
          ...
          if (open != -1) {
            ...
            if (1 != sscanf(foldnum, "%2d", &level)) level = -1;

            if (!isdigit(LL[l]->Chars[pos + 4]) ||
                !isdigit(LL[l]->Chars[pos + 5])) level = -1;
          ...
          if ((BFI(this,
            ...
            while (pos + len_end + 6 + 6 <= LL[l]->Count) {
              ...
              if ((pos + i + 6 + len_end > (unsigned int)LL[l]->Count) ||
                  (i == 0)) {
                error = 1;
                break;
              }
          ...
          if (open != -1) {
            ...
            if (open == 0) if (FoldClose(l) == 0) goto fail;
          }
  ...
  if (stat(FileName, &FileStatus) == -1) {
    ...
    if (!(FileStatus.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH))) BFI(this,
                                                                   BFI_ReadOnly)
        = 1;
    else BFI(this, BFI_ReadOnly) = 0;
  }
  close(fd);
...
# else // if defined(__IBMCPP__) || defined(__WATCOMC__)
    if (ftruncate(fileno(fp), 0) != 0)
# endif // __IBMCPP__ || __WATCOMC__goto erroropen;
#endif // OS2

  if (fp == 0) fp = fopen(AFileName, "wb");
  ...
  for (l = 0; l < RCount; l++) {
    ...
    if ((BFI(this,
      ...
      while ((bindex = GetUserBookmarkForLine(bindex, l, bname, bpos)) != -1) {
        ...
        if ((strlen(bname) > 256) ||
            (blen + strlen(bname) + 6 + 6 > sizeof(book))) continue;
        blen +=
          sprintf(book + blen, "%04x%02x%s", bpos.Col, (unsigned int)strlen(
                    bname), bname);
      }
    ...
    for (int what = 1; what < 3; what++) {
      if (((BFI(this, BFI_SaveFolds) == what) && foldlen) ||
        ...
        if (len_start) {
          if (fwrite(BFS(this,
                         BFS_CommentStart), 1, len_start,
                     fp) != len_start) goto fail;
          ByteCount += len_start;
        }
        ...
        if (len_end) {
          if (fwrite(BFS(this,
                         BFS_CommentEnd), 1, len_end, fp) != len_end) goto fail;
          ByteCount += len_end;
        }
      }
      ...
      if (what == 1) {
        // write data
        if ((int)(fwrite(L->Chars, 1, L->Count, fp)) != L->Count) goto fail;
        ByteCount += L->Count;
      }
    }
