      ...
      case hsREXX_Normal:
        ...
        if (isalpha(*p) || (*p == '_')) {
          ...
          if (State == hsREXX_Keyword)
            if (strnicmp(Line->Chars + i, "CALL", 4) == 0) wascall = 1;
          i    += j;
          len  -= j;
          p    += j;
          C    += j;
...
static int Match(int         Len,
  ...
  if (Pos + L <= Len)
    if ((StateMap == NULL) || IsState(StateMap + Pos, State, L))
      if (strnicmp(String, Text + Pos, L) == 0) return 1;

  return 0;
}

static int Match2(int         Len,
                  int         Pos,
                  hsState    *StateMap,
                  const char *Text,
                  const char *String,
  ...
  for (i = 0; i < Pos; i++) if (Text[i] != ' ') return 0;
...
static int SearchMatch(int Count, EBuffer *B, int Row, int Ctx) {
  ...
  // We try to find the indent of the line that "started" this "block" and
  // return it.
  while (Row >= 0) {
    P        = B->RLine(Row)->Chars;
    L        = B->RLine(Row)->Count;
          ...
          if ((Ctx == 1) || (Ctx == 3)) {
            if (Match(L, Pos, StateMap, P, "do", hsREXX_Keyword) ||
              ...
              if (Count == 0) {
                ...
                if (Ctx == 3) return B->LineIndented(Row);
                else return B->LineIndented(Row) + REXX_BASE_INDENT;
              }
...
static int CheckLabel(EBuffer *B, int Line) {
  ...
  if ((Line > 0) && (B->RLine(Line - 1)->StateE != hsREXX_Normal)) return 0;

  while ((P < L->Count) &&
         (L->Chars[P] == ' ' || L->Chars[P] == 9)) P++;

  while (P < L->Count) {
    if (Cnt > 0) if (L->Chars[P] == ':') return 1;
  ...
  while (Row >= 0) {
    ...
    if (L > 0) while (Pos >= 0) {
        ...
        if (isalpha(P[Pos]) && ((Pos == 0) || !isalpha(P[Pos - 1]))) {
          if (Match(L, Pos, StateMap, P, "do",
                    hsREXX_Keyword) ||
                     Match(L, Pos, StateMap, P, "loop", hsREXX_Keyword)) {
            Count++;
            ChFind = 'd';

            // } else if (Match(L, Pos, StateMap, P, "procedure",
            // hsREXX_Keyword)) {
            // Count++;
            // ChFind = 'p';
            ...
            if (StateMap) free(StateMap);
            return B->LineIndented(Row) + REXX_BASE_INDENT;
          } else if (Match(L, Pos, StateMap, P, "otherwise",
            ...
            ChFind = 'e';
                     ...
                     Match(L, Pos, StateMap, P, "then", hsREXX_Keyword)) {
            ChFind = 't';

            if (StateMap) free(StateMap);
            return B->LineIndented(Row);
          } else if ((is_blank < 5) &&
                     Match(L, Pos, StateMap, P, "else", hsREXX_Keyword)) {
            ChFind = 'e';
...
static int IndentNormal(EBuffer *B, int Line, int /*StateLen*/,
  ...
  if (CheckLabel(B, Line)) {
  ...
  } else if (LookAtNoCase(B, Line, 0, "method",
                          hsREXX_Keyword) ||
    ...
    case 'p':
      ...
      if (LookAtNoCase(B, Line, 0, "return", hsREXX_Keyword)) return I;
      else return I + REXX_BASE_INDENT;
    ...
    case 'o':
      ...
      if (LookAtNoCase(B, Line, 0, "do",
                       hsREXX_Keyword)) return I + REXX_DO_OFFSET;
      else return I + REXX_BASE_INDENT;
