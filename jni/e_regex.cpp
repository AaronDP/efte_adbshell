...
static int CountWidth(RxNode *N) {
  ...
  while (N) {
    if (N->fWhat < 32) w += 0;
    else if ((N->fWhat >= 32) && (N->fWhat < 64)) w += 1;
    N = N->fNext;
  }
  return w;
}
...
int RxMatch(RxNode *rx) {
  ...
  while (n) {
    ...
    case RE_CHAR:
      ...
      if (flags & RX_CASE) {
        ...
        for (int i = 0; i < n->fLen; i++)
          if (toupper(rex[i]) != toupper(n->fChar[i])) return 0;
      }
      rex += n->fLen;
      break;
    ...
    case RE_INSET:
      ...
      if ((n->fChar[(unsigned char)(*rex) >>
                    3] & (1 << ((unsigned char)(*rex) & 7))) == 0) return 0;
    ...
    case RE_NOTINSET:
      ...
      if (n->fChar[(unsigned char)(*rex) >>
                   3] & (1 << ((unsigned char)(*rex) & 7))) return 0;
      ...
      if (n->fWhat & RE_GROUP) {
        if (n->fWhat & RE_MEM) {
          ...
          if (RxMatch(n->fNext) == 0) {
            ...
            if (n->fWhat & RE_OPEN) match->Open[b] = -1;
            else match->Close[b] = -1;
            return 0;
          }
...
int RxExec(RxNode      *Regexp,
  ...
  case RE_CHAR:            // search for a character to match
    ...
    if (flags & RX_CASE) {
      while (1) {
        while (Start < eop && *Start != Ch) Start++;
      ...
      while (1) {
        while (Start < eop && toupper(*Start) != Ch) Start++;
...
int RxReplace(const char *rep,
  ...
  *Dlen = 0;
  ...
  while (*rep) {
    switch (Ch = *rep++) {
    //        case '&':
    //            add(&dlen, &dest, Src + match.Open[0], match.Close[0] -
    // match.Open[0], flag);
    //            break;
    case '\\':
