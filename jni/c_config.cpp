...
static int SetModeString(EMode *mode, int what, const char *string) {
  ...
  if (j == BFI_EventMap) {
    ...
    if (mode->MatchName) free(mode->MatchName);

    if (mode->MatchNameRx) RxFree(mode->MatchNameRx);
    mode->MatchName   = strdup(string);
    mode->MatchNameRx = RxCompile(string);
  } else if (j == BFS_FirstLineRx) {
    if (mode->MatchLine) free(mode->MatchLine);

    if (mode->MatchLineRx) RxFree(mode->MatchLineRx);
    mode->MatchLine   = strdup(string);
    mode->MatchLineRx = RxCompile(string);
  } else {
    if (mode->Flags.str[j & 0xFF]) free(mode->Flags.str[j & 0xFF]);
...
static int GetNum(CurPos& cp, long& num) {
  ...
  if ((n[3] > 127) && (sizeof(long) > 4)) num = num | (~0xFFFFFFFFUL);
  cpos++;
  return 1;
}
...
static int ReadHilitColors(CurPos& cp, EColorize *Colorize,
  ...
  while ((obj = GetObj(cp, len)) != 0xFF) {
    ...
    case CF_INT: {
      ...
      if (Colorize->SetColor(cidx, svalue) == 0) return -1;
...
static int ReadColorize(CurPos& cp, EColorize *Colorize, const char *ModeName) {
  ...
  while ((obj = GetObj(cp, len)) != 0xFF) {
    ...
    case CF_HSTATE: {
      ...
      if (Colorize->hm == 0) Colorize->hm = new HMachine();

        assert(Colorize->hm != 0);

      if (GetNum(cp, stateno) == 0) return -1;
    ...
    case CF_HTRANS: {
      ...
      if (GetNum(cp, nextState) == 0) return -1;

      obj = GetObj(cp, len);
        assert(obj == CF_INT);

      if (GetNum(cp, matchFlags) == 0) return -1;
      ...
      if ((match = GetCharStr(cp, len)) == 0) return -1;
    ...
    case CF_HWTYPE: {
      ...
      if (GetNum(cp, nextKwdMatchedState) == 0) return -1;

      obj = GetObj(cp, len);
        assert(obj == CF_INT);

      if (GetNum(cp, nextKwdNotMatchedState) == 0) return -1;

      obj = GetObj(cp, len);
        assert(obj == CF_INT);

      if (GetNum(cp, nextKwdNoCharState) == 0) return -1;

      obj = GetObj(cp, len);
        assert(obj == CF_INT);

      if (GetNum(cp, options) == 0) return -1;

      obj = GetObj(cp, len);
        assert(obj == CF_STRING);

      if ((wordChars = GetCharStr(cp, len)) == 0) return -1;
    ...
    case CF_HWORDS: {
      ...
      if (ReadKeywords(cp, &Colorize->hm->LastState()->keywords,
                       color) == -1) return -1;
...
static int ReadConfigFile(CurPos& cp) {
  ...
  while ((obj = GetObj(cp, len)) != 0xFF) {
    ...
    case CF_EVENTMAP: {
      ...
      if (len > 0) if ((UpMap = GetCharStr(cp, len)) == 0) return -1;

      // add new mode
      if ((EventMap = FindEventMap(MapName)) == 0) {
        EEventMap *OrgMap = 0;

        if (strcmp(UpMap, "") != 0) OrgMap = FindEventMap(UpMap);
        EventMap = new EEventMap(MapName, OrgMap);
      } else {
        if (EventMap->Parent == 0) EventMap->Parent = FindEventMap(UpMap);
      }
    ...
    case CF_COLORIZE: {
      ...
      if (len > 0) if ((UpMode = GetCharStr(cp, len)) == 0) return -1;

      // add new mode
      if ((Mode = FindColorizer(ModeName)) == 0) Mode = new EColorize(ModeName,
                                                                      UpMode);
      else {
        if (Mode->Parent == 0) Mode->Parent = FindColorizer(UpMode);
      }

      if (ReadColorize(cp, Mode, ModeName) == -1) return -1;
    ...
    case CF_MODE: {
      ...
      if ((Mode = FindMode(ModeName)) == 0) {
        ...
        if (strcmp(UpMode, "") != 0) OrgMode = FindMode(UpMode);
        Map = FindEventMap(ModeName);

        if (Map == 0) {
          EEventMap *OrgMap = 0;

          if (strcmp(UpMode, "") != 0) OrgMap = FindEventMap(UpMode);
        ...
        if (Mode->fParent == 0) Mode->fParent = FindMode(UpMode);
      }

      if (ReadMode(cp, Mode, ModeName) == -1) return -1;
    ...
    case CF_OBJECT: {
      ...
      if ((ObjName = GetCharStr(cp, len)) == 0) return -1;

      if (ReadObject(cp, ObjName) == -1) return -1;
