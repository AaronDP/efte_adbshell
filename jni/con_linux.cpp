...
int ConInit(int /*XSize*/, int /*YSize*/) {
  ...
  if (MAJOR(stb.st_rdev) != TTY_MAJOR) die("Not running in a virtual console.");

  if (ioctl(VtFd, KDGKBMODE, &mode) != 0) die("failed to get kbdmode");
#if 0

  if (mode != K_XLATE) { // X, etc...
    sprintf(ttyname, "/dev/console");

    if ((VtFd = open(ttyname, O_RDWR)) == -1) die("failed to open /dev/console");

    if ((ioctl(VtFd, VT_OPENQRY, &free_tty) < 0) || (free_tty == -1)) die(
        "could not find a free tty\n");
    close(VtFd);
    VtNum = free_tty;
    sprintf(ttyname, "/dev/tty%d", VtNum);

    if ((VtFd = open(ttyname, O_RDWR)) == -1) die("could not open tty");
  }
#endif // if 0
  ...
  if (ioctl(VtFd, GIO_SCRNMAP, &toScreen) == -1) die(
      "could not get screen character mapping!");

  {
    int c;

    for (c = 0; c < 256; c++) fromScreen[c] = 0;

    for (c = 0; c < 256; c++) fromScreen[toScreen[c]] = c;
  }
#endif // ifdef USE_SCRNMAP
...
int ConGetEvent(TEventMask /*EventMask*/, TEvent *Event, int WaitTime,
  ...
  if (GpmFd != -1) FD_SET(GpmFd, &readfds);
#endif // ifdef USE_GPM

  for (int p = 0; p < MAX_PIPES; p++)
    if (Pipes[p].used)
      if (Pipes[p].fd != -1) FD_SET(Pipes[p].fd, &readfds);
...
int GetKeyEvent(TEvent *Event) {
  ...
  if (read(VtFd, &keycode, 1) != 1) return -1;
  ...
  case KT_SHIFT:
    ...
    if (ksv == KG_SHIFT) {
      if (key == 54) ksv = KG_SHIFTR;
      else ksv = KG_SHIFTL;
    } else if (ksv == KG_CTRL) {
      if (key == 97) ksv = KG_CTRLR;
      else ksv = KG_CTRLL;
    }

    if (press) shift_state |= (1 << ksv);
    else shift_state &= ~(1 << ksv);
    break;
  ...
  case KT_SLOCK:
    ...
    if (press) slock_state |= (1 << ksv);
    else slock_state &= ~(1 << ksv);
    break;
  }
  ...
  if ((key == 14) && (keysym == K(KT_LATIN, 127))) {
    ...
    KeyCode = kbBackSp;
  } else for (unsigned int i = 0; i < sizeof(KeyTrans) / sizeof(KeyTrans[0]);
              i++) {
  ...
  if (KeyCode == 0) {
    ...
    case KT_LATIN:
      ...
      if (ShiftFlags & kfAlt) KeyCode = toupper(KeyCode);

      if (KTYP(keysym) == KT_DEAD) {
        for (int i = 0; (unsigned)i < sizeof(DeadTrans) / sizeof(DeadTrans[0]);
             i++) {
  ...
  if (!press) KeyCode |= kfKeyUp;

  if (KeyCode != 0) {
    if (KTYP(keysym) != KT_SHIFT) dead_key = 0;

    if (KeyCode & kfKeyUp) Event->What = evKeyUp;
...
int GetMouseEvent(TEvent *Event) {
  ...
  if (rc == 0) {
    ...
    // Gpm_FitEvent(&e);
    if ((e.type & GPM_MOVE) || (e.type & GPM_DRAG)) Event->What = evMouseMove;
    else if (e.type & GPM_DOWN) Event->What = evMouseDown;
    else if (e.type & GPM_UP) Event->What = evMouseUp;
    else return 0;
