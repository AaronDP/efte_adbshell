...
ESvnLog::ESvnLog(int         createFlags,
                 EModel    **ARoot,
                 const char *Directory,
                 const char *AOnFiles) : EBuffer(createFlags, ARoot, NULL) {
  int   i, j, p;
...
int ESvnLog::ConfQuit(GxView *V, int /*multiFile*/) {
  ...
  switch (V->Choice(GPC_ERROR, "SVN commit pending", 3, "C&ommit", "&Discard",
                    "&Cancel", "")) {
  case 0: // Commit
...
void ESvnLog::GetTitle(char *ATitle, int MaxLen, char *ASTitle,
                       int SMaxLen) const {
