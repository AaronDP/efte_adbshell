  ...
  if (Event.What & evMouse) {
    ...
    if ((Event.What != evMouseDown) || (Event.Mouse.Y == H - 1)) {
      ...
      case evMouseDown:
        ...
        if (CaptureMouse(1)) MouseCaptured = 1;
        else break;
        Event.What = evNone;
        break;
      ...
      case evMouseMove:
        ...
        if (MouseCaptured) {
          if (Event.Mouse.Y != H - 1) ExpandHeight(Event.Mouse.Y - H + 1);
          Event.What = evNone;
        }
        break;
      ...
      case evMouseAuto:
        ...
        if (MouseCaptured) Event.What = evNone;
        break;

      case evMouseUp:

        if (MouseCaptured) CaptureMouse(0);
        else break;
        MouseCaptured = 0;
        Event.What    = evNone;
        break;
...
int GxView::GetFile(const char  *Prompt,
                    unsigned int BufLen,
                    char        *Str,
                    int          HistId,
                    int          Flags) {
  if ((HaveGUIDialogs & GUIDLG_FILE) && GUIDialogs) return DLGGetFile(this,
                                                                      Prompt,
                                                                      BufLen,
                                                                      Str,
                                                                      Flags);
  else return ReadStr(Prompt, BufLen, Str, CompletePath, SelectPathname, HistId);
}
...
int GxView::ReadStr(const char  *Prompt,
                    unsigned int BufLen,
                    char        *Str,
                    Completer    Comp,
                    int          Select,
...
int GxView::IncrementalSearch(EView *View) {
  ...
  if (View->GetContext() != CONTEXT_FILE) return 0;
