...
EMessages::EMessages(int         createFlags,
                     EModel    **ARoot,
                     const char *ADir,
                     const char *ACommand) : EList(createFlags, ARoot,
                                                   "Messages") {
  CompilerMsgs = this;
  ErrCount     = 0;
  ErrList      = 0;
...
void EMessages::FindErrorFiles() {
  for (int i = 0; i < ErrCount; i++)
    if ((ErrList[i]->Buf == 0) && (ErrList[i]->file != 0)) FindErrorFile(i);
}

void EMessages::FindErrorFile(int err) {
  assert(err >= 0 && err < ErrCount);

  if (ErrList[err]->file == 0) return;
...
void EMessages::AddFileError(EBuffer *B, int err) {
  ...
  if (P.Row >= B->RCount) P.Row = B->RCount - 1;

  if (P.Row < 0) P.Row = 0;

  if (B->PlaceBookmark(bk, P) == 1) ErrList[err]->Buf = B;
}
...
void EMessages::AddError(Error *p) {
  ErrCount++;
  ErrList                    = (Error **)realloc(ErrList,
                                                 sizeof(void *) * ErrCount);
...
void EMessages::AddError(const char *file,
                         int         line,
                         const char *msg,
                         const char *text,
                         int         hilit) {
...
int EMessages::GetLine(char *Line, int maxim) {
  ...
  *Line = 0;
  ...
  if (BufLen - BufPos < l) l = BufLen - BufPos;
    ...
    for (i = 0; i < NCRegexp; i++) {
      if (RxExec(CRegexp[i].rx, line, strlen(line), line, &RM) == 1) {
        ...
        *fn = 0;
        ...
        if (RM.Close[n] - RM.Open[n] < 256) memcpy(fn,
                                                   line + RM.Open[n],
                                                   RM.Close[n] - RM.Open[n]);
        n = CRegexp[i].RefLine;

        if (RM.Close[n] - RM.Open[n] < 256) memcpy(ln,
                                                   line + RM.Open[n],
                                                   RM.Close[n] - RM.Open[n]);
        n = CRegexp[i].RefMsg;

        if (RM.Close[n] - RM.Open[n] < 256) memcpy(msg,
                                                   line + RM.Open[n],
                                                   RM.Close[n] - RM.Open[n]);

        if (IsFullPath(fn)) file = fn;
        else {
          if (curr_dir == 0) strcpy(fn1, Directory);
          else strcpy(fn1, curr_dir->name);
          Slash(fn1, 1);
          strcat(fn1, fn);

          if (ExpandPath(fn1, fn2, sizeof(fn2)) == 0) file = fn2;
          else file = fn1;
        }
    ...
    if (!didmatch) {
      ...
      if ((pin = strstr(line, "]:")) != 0) {
        ...
        if (strnicmp(pin, t1, sizeof(t1) - 1) == 0) { // Entering?
          ...
          if (a != 0) curr_dir = curr_dir->next;  // Remove from stack,
...
bad:
    ...
    if (Row < ErrCount - 1) {
      ...
      if (Running) V->Msg(S_INFO, "No more errors (yet).");
      else V->Msg(S_INFO, "No more errors.");
      return 0;
    }
  } else {
    if (Running) V->Msg(S_INFO, "No errors (yet).");
    else V->Msg(S_INFO, "No errors.");
    return 0;
  }
  return 1;
}
...
void EMessages::ShowError(EView *V, int err) {
  if ((err >= 0) && (err < ErrCount)) {
    if (ErrList[err]->file) {
      ...
      else V->SetMsg(ErrList[err]->text);
    }
  }
}
...
char * EMessages::FormatLine(int Line) const {
  ...
  if (Line < ErrCount) p = strdup(ErrList[Line]->text);
  else p = 0;
  return p;
}
...
int EMessages::CanActivate(int Line) const {
  ...
  if (Line <
      ErrCount) if (ErrList[Line]->file || (ErrList[Line]->line != -1)) ok = 1;
  return ok;
}

void EMessages::NotifyPipe(int APipeId) {
  // fprintf(stderr, "Got notified");
  if (APipeId == PipeId) GetErrors();
}
...
// get row length for specified row, used in MoveLineEnd to get actual row
// length
