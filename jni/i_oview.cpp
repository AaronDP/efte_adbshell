...
void ExView::HandleEvent(TEvent& Event) {
  if ((Event.What == evKeyDown) &&
      (kbCode(Event.Key.Code) == kbF12)) Win->Parent->SelectNext(0);
}
...
int ExView::ConPutBox(int X, int Y, int W, int H, PCell Cell) {
  if (Win) return Win->ConPutBox(X, Y, W, H, Cell);
...
int ExView::ConScroll(int Way, int X, int Y, int W, int H, TAttr Fill,
                      int Count) {
  if (Win) return Win->ConScroll(Way, X, Y, W, H, Fill, Count);

  return -1;
}

int ExView::ConQuerySize(int *X, int *Y) {
  if (Win) return Win->ConQuerySize(X, Y);

  return -1;
}

int ExView::ConSetCursorPos(int X, int Y) {
  if (Win) return Win->ConSetCursorPos(X, Y);

  return -1;
}

int ExView::ConShowCursor() {
  if (Win) return Win->ConShowCursor();

  return -1;
}

int ExView::ConHideCursor() {
  if (Win) return Win->ConHideCursor();

  return -1;
}

void ExView::ConSetInsertState(bool insert) {
  if (Win) Win->ConSetInsertState(insert);
}
