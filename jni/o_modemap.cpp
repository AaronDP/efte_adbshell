...
void EventMapView::DumpKey(const char *aPrefix, EKey *Key) {
  ...
  GetKeyName(KeyName + strlen(KeyName), sizeof(KeyName) - strlen(
               KeyName), Key->fKey);
  ...
  for (int i = 0; i < Macros[id].Count; i++) {
    ...
    if (Macros[id].cmds[i].type == CT_COMMAND) {
      if (Macros[id].cmds[i].repeat > 1) sprintf(p,
                                                 "%d:%s ",
                                                 Macros[id].cmds[i].repeat,
                                                 GetCommandName(Macros[id].cmds[i
                                                                ].u.num));
      else sprintf(p, "%s ", GetCommandName(Macros[id].cmds[i].u.num));
...
void EventMapView::DumpMap(const char *aPrefix, EKeyMap *aKeyMap) {
  ...
  while (Key) {
    if (Key->fKeyMap) {
      ...
      GetKeyName(Prefix + strlen(Prefix), sizeof(Prefix) - strlen(
                   Prefix), Key->fKey);
...
void EventMapView::DumpEventMap(EEventMap *aEventMap) {
  ...
  while (aEventMap) {
    ...
    if (aEventMap->KeyMap) DumpMap(0, aEventMap->KeyMap);
    aEventMap = aEventMap->Parent;

    if (aEventMap != 0) AddLine("");
  }
}

EventMapView::EventMapView(int        createFlags,
                           EModel   **ARoot,
...
void EventMapView::DrawLine(PCell B, int Line, int Col, ChColor color,
                            int Width) {
  if (Line < BCount)
    if (Col < int(strlen(BList[Line]))) MoveStr(B,
                                                0,
                                                Width,
                                                BList[Line] + Col,
                                                color,
...
void EventMapView::GetTitle(char *ATitle,
                            int /*MaxLen*/,
                            char *ASTitle,
                            int   SMaxLen) const {
