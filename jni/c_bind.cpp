...
const char* GetCommandName(int Command) {
  if (Command & CMD_EXT) {
    ...
    if ((Command < 0) ||
        (Command >= CMacros)) return "?INVALID?";

    if (Macros[Command].Name) return Macros[Command].Name;
    else return "?NONE?";
  }

  for (int i = 0; i < int(sizeof(Command_Table) / sizeof(Command_Table[0])); i++)
    if (Command_Table[i].CmdId == Command) return Command_Table[i].Name;
...
int CmdNum(char *Cmd) {
  ...
  for (i = 0;
       ...
       i++)
    if (strcmp(Cmd, Command_Table[i].Name) == 0) return Command_Table[i].CmdId;

  for (i = 0; i < CMacros; i++)
    if (Macros[i].Name &&
        ((strcmp(Cmd, Macros[i].Name)) == 0)) return i | CMD_EXT;
...
EMode* FindMode(const char *Name) {
  ...
  while (m) {
    if (strcmp(Name, m->fName) == 0) return m;
...
EEventMap* FindActiveMap(EMode *Mode) {
  while (Mode) {
    if (Mode->fEventMap) return Mode->fEventMap;
  ...
  while (d) {
    ...
    if (d == 0) {
      ...
      while (pm) { // while exists
        ...
        if ((pk = pm->FindKey(ks.Key)) != 0) { // if key exists, find parent of
                                               // it
          parent = pk->fKeyMap;
  ...
  if (init == 0) {
    for (int i = 0; i < 256; i++)
      ...
      if (isalnum(i) || (i == '_')) {
        ...
        // even faster.
        if (/*(i >= 'A' && i <= 'Z') || */ isupper(i)) WSETBIT(
            DefaultBufferFlags.CapitalChars,
            i,
            1);
      }
    init = 1;
...
void SetWordChars(char *w, const char *s) {
  ...
  while (p && *p) {
    if (*p == '\\') {
      ...
      for (int i = p[0]; i < p[2]; i++) WSETBIT(w, i, 1);
      p += 2;
    }
    WSETBIT(w, *p, 1);
    p++;
...
EMode::EMode(EMode      *aMode,
  ...
  if (aMode) {
    ...
    for (int i = 0; i < BFS_COUNT; i++) {
      if (aMode->Flags.str[i] != 0) Flags.str[i] = strdup(aMode->Flags.str[i]);
    }
...
EMode::~EMode() {
  ...
  for (int i = 0; i < BFS_COUNT; i++) free(Flags.str[i]);
}

void EMode::AddIndentRx(int         look_line,
                        int         affect_line,
                        int         indent,
                        const char *regex,
                        int         flags) {
  if (indent_count > 20) return;
...
static int MatchKey(TKeyCode aKey, KeySel aSel) {
  ...
  if (flags & kfShift) {
    if (key < 256) {
      if (flags == kfShift) flags &= ~kfShift;
      else if (isascii(key)) key = toupper(key);
    }
  }
...
EEventMap::EEventMap(const char *AName,
                     EEventMap  *AParent) {
  Name      = strdup(AName);
  Parent    = AParent;
  KeyMap    = 0;
...
void EEventMap::SetMenu(int which, const char *What) {
  if ((which < 0) || (which >= EM_MENUS)) return;

  if (Menu[which] != 0) free(Menu[which]);
  Menu[which] = strdup(What);
}

char * EEventMap::GetMenu(int which) {
  if ((which < 0) || (which >= EM_MENUS)) return 0;

  if (Menu[which] || (Parent == 0)) return Menu[which];
  else return Parent->GetMenu(which);
}
...
EAbbrev * EMode::FindAbbrev(const char *string) {
  ...
  while (Map) {
    ...
    while (ab != 0) {
      if (ab->Match && (strcmp(string, ab->Match) == 0)) return ab;
...
int ParseKey(const char *Key, KeySel& ks) {
  ...
  if (ks.Key == 0) ks.Key = *p;

  if ((KeyFlags & kfCtrl) && !(KeyFlags & kfSpecial)) {
    if (ks.Key < 256) {
      if (ks.Key < 32) ks.Key += 64;
      else ks.Key = toupper(ks.Key);
    }
  }
  ks.Key |= KeyFlags;
  return 0;
...
int GetKeyName(char *Key, int KeySize, KeySel& ks) {
  ...
  if (keyCode(ks.Key) < 256) {
    ...
    //    if (c[0] < ' ')
    //        c[0] += '@';
    if (c[0] == 32) strlcat(Key, "Space", KeySize);
    else strlcat(Key, c, KeySize);
  } else {
...
EAbbrev::EAbbrev(const char *aMatch,
                 const char *aReplace) {
  next    = 0;
  Match   = strdup(aMatch);
...
int AddCommand(int no, int Command, int count, int ign) {
  ...
  Macros[no].cmds                          = (CommandType *)realloc(
    Macros[no].cmds,
...
int NewCommand(const char *Name) {
  Macros =
    (ExMacro *)realloc(Macros, sizeof(ExMacro) * (1 + CMacros));
...
int ExState::GetStrParam(EView *view, char *str, int maxlen) {
  ...
  if ((Macro == -1)
      || (Pos == -1)
      || (Pos >= Macros[Macro].Count)) return 0;
  ...
  if (Macros[Macro].cmds[Pos].type == CT_STRING) {
    ...
    // puts("variable\x7");
    if (view->GetStrVar(Macros[Macro].cmds[Pos].u.num, str,
                        maxlen) == 0) return 0;
  ...
  if (Pos < Macros[Macro].Count) {
    if (Macros[Macro].cmds[Pos].type == CT_CONCAT) {
      ...
      // puts("concat\x7");
      if (GetStrParam(view, str + len, left) == 0) return 0;
    }
  }
  return 1;
}
...
int ExState::GetIntParam(EView *view, int *value) {
  ...
  if (Macros[Macro].cmds[Pos].type == CT_NUMBER) {
    *value = Macros[Macro].cmds[Pos].u.num;
    ...
    if (view->GetIntVar(Macros[Macro].cmds[Pos].u.num, value) == 0) return 0;
