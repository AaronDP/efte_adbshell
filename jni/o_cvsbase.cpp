...
int AddCvsIgnoreRegexp(const char *regexp) {
  ...
  if ((CvsIgnoreRegexp[CvsIgnoreRegexpCount] = RxCompile(regexp)) ==
      NULL) return 0;
...
ECvsBase::ECvsBase(int         createFlags,
                   EModel    **ARoot,
...
void ECvsBase::AddLine(const char *file, int line, const char *msg, int status) {
  ...
  if (l != 0) {
    ...
    LineCount++;
    Lines                = (CvsLine **)realloc(Lines,
                                               sizeof(CvsLine *) * LineCount);
    Lines[LineCount - 1] = l;
...
void ECvsBase::NotifyPipe(int APipeId) {
  if (APipeId == PipeId) {
    ...
    while (GetLine((char *)line, sizeof(line))) {
      ...
      for (i = 0; i < CvsIgnoreRegexpCount;
           i++) if (RxExec(CvsIgnoreRegexp[i], line, len, line, &RM) == 1) break;
...
int ECvsBase::RunPipe(const char *ADir,
                      const char *ACommand,
                      const char *AOnFiles) {
  free(Command);
  free(Directory);
  ...
  if ((line >= 0) && (line < LineCount) && Lines[line]->File) {
    if (Lines[line]->Buf != 0) {
      ...
      if (FileLoad(0, path, 0, V) == 1) {
        ...
        if (Lines[line]->Line != -1) ((EBuffer *)ActiveModel)->CenterNearPosR(
            0,
            Lines
            [line]->Line);
      }
...
void ECvsBase::GetTitle(char *ATitle, int MaxLen, char *ASTitle,
                        int SMaxLen) const {
  char format[128];
