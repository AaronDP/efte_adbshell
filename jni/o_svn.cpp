...
ESvn::ESvn(int         createFlags,
           EModel    **ARoot,
           const char *ADir,
           const char *ACommand,
           const char *AOnFiles) : ESvnBase(createFlags, ARoot, "SVN") {
  SvnView   = this;
  LogFile   = 0;
  Commiting = 0;
...
ESvn::ESvn(int      createFlags,
           EModel **ARoot) : ESvnBase(createFlags, ARoot, "SVN") {
  SvnView = this;
  LogFile = 0;
}
...
char * ESvn::MarkedAsList() {
  ...
  for (i = 0; i < LineCount; i++) if (Lines[i]->Status & 2) len += strlen(
        Lines[i]->File) + 1;
  ...
  for (i = 0; i < LineCount;
       i++) if (Lines[i]->Status & 2) strcat(strcat(s, Lines[i]->File), " ");
  s[strlen(s) - 1] = 0;
  return s;
}
...
char ESvn::GetFileStatus(const char *file) {
  ...
  for (int i = LineCount - 1; i >= 0;
       i--) if (Lines[i]->File &&
                (filecmp(Lines[i]->File, file) == 0)) return Lines[i]->Msg[0];
...
// Подсвечиваются и можно перейти на эти
// файлы
void ESvn::ParseLine(const char *line, int len) {
  if ((len > 2) && (line[1] == ' ') && strchr(SvnStatusChars, line[0])) AddLine(
      line + 7,
      -1,
      line,
      5);
...
int ESvn::RunCommit(const char *ADir, const char *ACommand,
                    const char *AOnFiles) {
  if (!SameDir(Directory, ADir)) FreeLines();
