...
EMark::EMark(const char *aName,
  ...
  if (aBuffer == 0) aBuffer = FindFile(aFileName);

  if (aBuffer && aBuffer->Loaded) SetBuffer(aBuffer);
  else aBuffer = 0;
}

EMark::~EMark() {
  if (Buffer) RemoveBuffer(Buffer);
  delete[] Name;
  delete[] FileName;
}
...
int EMark::SetBuffer(EBuffer *aBuffer) {
  ...
  if (Point.Row >= aBuffer->RCount) Point.Row = aBuffer->RCount - 1;
...
int EMark::RemoveBuffer(EBuffer *aBuffer) {
  ...
  if ((Buffer == 0) || (Buffer != aBuffer)) return 0;

    assert(filecmp(aBuffer->FileName, FileName) == 0);

  if (Buffer->GetBookmark(Name, Point) == 0) return 0;
...
EMarkIndex::~EMarkIndex() {
  if ((markCount > 0) && marks) {
    for (int n = 0; n < markCount; n++) delete marks[n];
    free(marks);
    marks = 0;
  }
}

EMark * EMarkIndex::Insert(const char *aName,
                           const char *aFileName,
                           EPoint      aPoint,
                           EBuffer    *aBuffer) {
...
int EMarkIndex::Remove(const char *aName) {
  ...
  while (L < R) {
    ...
    if (cmp == 0) {
      ...
      if ((newMarks != 0) || (markCount == 0)) marks = newMarks;
...
int EMarkIndex::View(EView *aView, const char *aName) {
  ...
  if (m) {
    ...
    if (b == 0) {
      if (FileLoad(0, m->GetFileName(), 0, aView) == 0) return 0;
...
int EMarkIndex::RetrieveForBuffer(EBuffer *aBuffer) {
  for (int n = 0; n < markCount; n++)
    if ((marks[n]->GetBuffer() == 0) &&
      ...
      if (marks[n]->SetBuffer(aBuffer) == 0) return 0;
    }
  return 1;
}
...
int EMarkIndex::StoreForBuffer(EBuffer *aBuffer) {
  for (int n = 0; n < markCount; n++)
    if (marks[n]->GetBuffer() == aBuffer)
      if (marks[n]->RemoveBuffer(aBuffer) == 0) return 0;
