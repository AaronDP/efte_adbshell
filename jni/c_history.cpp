...
int LoadHistory(const char *FileName) { /*FOLD00*/
  ...
  while (fgets(line, sizeof(line), fp) != 0) {
    ...
    } else if ((line[0] == 'B') && (line[1] == '|')) { // bookmark history for
                                                       // last file
      if (last) {
        int r, c;
        p = line + 2;
        ...
        *e = 0;
        nb =
          (HBookmark **)realloc(last->Books, sizeof(HBookmark *) *
                                (last->BookCount + 1));
...
int AddInputHistory(int Id, const char *String) { /*FOLD00*/
  char *s = NULL;                                 // get rid of a "might not be
                                                  // initialised" warning
  int   i;
...
int CountInputHistory(int Id) { /*FOLD00*/
  ...
  for (i = 0; i < inputHistory.Count; i++) if (inputHistory.Id[i] == Id) c++;
  return c;
}
...
int StoreBookmarks(EBuffer *buffer) { /*FOLD00*/
  ...
  while (L < R) {
    ...
    if (cmp == 0) {
      ...
      for (i = j = 0; (i = buffer->GetUserBookmarkForLine(i, -1, name, P)) >= 0;
           j++) ;
      ...
      if (j) {
        ...
        if (FPHistory[M]->Books) {
          for (i = j = 0;
               (i = buffer->GetUserBookmarkForLine(i, -1, name, P)) >= 0; j++) {
