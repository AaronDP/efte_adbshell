...
int Hilit_SH(EBuffer *BF,
    ...
    IF_TAB() else {
      ...
      case hsSH_Normal:
        ...
        if ((CommandStr == 1) && (len > 2) && (*p == '.') && isspace(p[1])) {
          ...
          if (p[j] == '=') Color = CLR_Variable;
          else if ((p[j] == '*') || (p[j] == ')')) Color = CLR_Normal;
          else {
          ...
          while (len > j && (isspace(p[j]) || p[j] == '\'' || p[j] == '"')) j++;

          if (p[j] == '\\') j++;

          while (len > j &&
                 !(isspace(p[j]) || p[j] == '\'' || p[j] == '"')) *s++ = p[j++];
  ...
  if ((State == hsSH_Comment) || (State == hsSH_Variable)) State = hsSH_Normal;
  *ECol = C;
  return 0;
}
