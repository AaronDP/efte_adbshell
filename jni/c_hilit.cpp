...
int GetIndentMode(const char *Str) {
  for (unsigned int i = 0; i < sizeof(IndentModes) / sizeof(IndentModes[0]); i++)
    if (strcmp(Str, IndentModes[i].Name) == 0) return IndentModes[i].Num;
...
int GetHilitMode(const char *Str) {
  for (unsigned int i = 0; i < sizeof(HilitModes) / sizeof(HilitModes[0]); i++)
    if (strcmp(Str, HilitModes[i].Name) == 0) return HilitModes[i].Num;
...
SyntaxProc GetHilitProc(int id) {
  for (unsigned int i = 0; i < sizeof(HilitModes) / sizeof(HilitModes[0]); i++)
    if (id == HilitModes[i].Num) return HilitModes[i].Proc;

  return 0;
}

int EBuffer::HilitAddWord(const char *Word) {
  if (HilitFindWord(Word) == 1) return 1;
...
EColorize::EColorize(const char *AName,
                     const char *AParent) {
  Name         = strdup(AName);
...
EColorize::~EColorize() {
  ...
  for (int i = 0; i < CK_MAXLEN; i++) free(Keywords.key[i]);
...
EColorize* FindColorizer(const char *AName) {
  ...
  while (p) {
    if (strcmp(AName, p->Name) == 0) return p;
...
int HState::GetHilitWord(int len, char *str, ChColor& clr) {
  ...
  if ((len >= CK_MAXLEN) || (len < 1)) return 0;
...
HMachine::~HMachine() {
  ...
  if (state) {
    ...
    while (stateCount--) {
      for (i = 0; i < CK_MAXLEN; i++) free(state[stateCount].keywords.key[i]);
