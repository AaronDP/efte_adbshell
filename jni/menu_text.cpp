...
static int GetVSize(int id, int& X, int& Y) {
  ...
  for (int i = 0; i < Y; i++) {
    ...
    if (Menus[id].Items[i].Name) len = CStrLen(Menus[id].Items[i].Name);

    if (len > xsize) xsize = len;
  }
  X = xsize;
  return 0;
}
...
static int DrawVMenu(int x, int y, int id, int active) {
  ...
  for (i = 0; i < Menus[id].Count; i++) {
    ...
    if (Menus[id].Items[i].Name) {
      ...
      if (arg) MoveCStr(B, w - len2 - 2, w + 4, arg, color1, color2, len2);
...
int ExecVertMenu(int x, int y, int id, TEvent& E, UpMenu *up) {
  ...
  while (abort == -2) {
    ...
    do {
      ConGetEvent(
        evCommand | evMouseDown | evMouseMove | evMouseUp | evKeyDown | evNotify,
        &E,
        -1,
        1);

      if (E.What & evNotify) gui->DispatchEvent(frames, frames->Active, E);
          ...
          if (wasmouse) {
            while (p) {
              if ((E.Mouse.X >= p->x) && (E.Mouse.Y >= p->y) &&
                ...
                if (first == 1) {
                  if (p->vert) {
                    int i = GetVPosItem(p->id,
                                        p->w,
                                        E.Mouse.X - p->x,
                                        E.Mouse.Y - p->y);

                    if (i != -1) if (Menus[p->id].Items[i].SubMenu == id) break;
                  } else {
...
int ExecMainMenu(TEvent& E, char sub) {
  ...
  while (abort == -2) {
    ...
    case evMouseMove:
      ...
      if (E.Mouse.Buttons) {
        if (E.Mouse.Y == 0) cur = GetHPosItem(id, E.Mouse.X);
        else cur = -1;
        dovert = 1;
      }
      break;
