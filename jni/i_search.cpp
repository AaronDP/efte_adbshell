...
ExISearch::~ExISearch() {
  if (ISearchStr[0] != 0) strcpy(PrevISearch, ISearchStr);
}
...
void ExISearch::HandleEvent(TEvent& Event) {
  ...
  case evKeyDown:
    ...
    case kbBackSp:
      ...
      if (len > 0) {
        if (stacklen > 0) {
          ...
          if (Buffer->CenterPos(stack[stacklen].Col,
                                stack[stacklen].Row) == 0) return;
        }
        len--;
        ISearchStr[len] = 0;

        if ((len > 0) &&
            (Buffer->FindStr(ISearchStr, len, Case | Direction) == 0)) {
          SetState(INoMatch);
        }
      } else {
    ...
    case kbTab | kfShift:
      ...
      if (Buffer->FindStr(ISearchStr, len,
