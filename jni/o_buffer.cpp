...
EViewPort * EBuffer::CreateViewPort(EView *V) {
  ...
  if ((Loaded == 0) && !suspendLoads) {
    ...
    if (CompilerMsgs) CompilerMsgs->FindFileErrors(this);
    ...
    if (RetrieveFPos(FileName, r, c) == 1) SetNearPosR(c, r);
...
void EEditPort::ScrollY(int Delta) {
  ...
  if ((Delta >= Rows) || (-Delta >= Rows)) return;
...
void EEditPort::DrawLine(int L, TDrawBuffer B) {
  ...
  if (View->MView->Win->GetViewContext() == View->MView) View->MView->ConPutBox(
      0,
      L
      - TP.Row,
      Cols,
...
void EEditPort::HandleMouse(TEvent& Event) {
  ...
  if ((Event.What != evMouseDown) || (y < H - 1)) {
    ...
    case evMouseDown:
      ...
      if (Event.Mouse.Y == H - 1) break;

      if (View->MView->Win->CaptureMouse(1)) View->MView->MouseCaptured = 1;
      else break;
      ...
      if (View->MView->MouseCaptured) {
        if (Event.Mouse.Buttons == 1) {
          ...
          if (!View->MView->MouseMoved) {
            ...
            if (Event.What == evMouseMove) View->MView->MouseMoved = 1;
          }
          ...
          if (Event.Mouse.KeyMask == kfAlt) {} else {
            Buffer->SetNearPos(xx, yy);
          }
        }
    ...
    case evMouseUp:
      ...
      if (View->MView->MouseCaptured) View->MView->Win->CaptureMouse(0);
      else break;
      ...
      if (Event.Mouse.Buttons == 2) {
        ...
        if (!View->MView->MouseMoved) {
          ...
          if (Map) MName = Map->GetMenu(EM_LocalMenu);

          if (MName == 0) MName = "Local";
      ...
      if (Event.Mouse.Buttons == 4) {
        ...
        if (SystemClipboard) {
          ...
          if (Event.Mouse.KeyMask == 0) Buffer->BlockPasteStream(1);
          else if (Event.Mouse.KeyMask == kfCtrl) Buffer->BlockPasteColumn(1);
          else if (Event.Mouse.KeyMask == kfAlt) Buffer->BlockPasteLine(1);
        }
      }
      Event.What = evNone;
      break;
...
int EBuffer::ExecCommand(int Command, ExState& State) {
  ...
  case ExInsertSpacesToTab: {
    ...
    if (State.GetIntParam(View, &no) == 0) no = 0;
...
int EBuffer::GetString(ExState& State) {
  ...
  if (View->MView->Win->GetStr(Prompt, sizeof(GetStrVars[No]), GetStrVars[No],
                               HIST_POSITION) == 0) return 0;
...
int EBuffer::RegExp(ExState& State) {
  ...
  /*
     ...
     if (RxReplace(Replace, Haystack, strlen(Haystack), match, &dest, &dest_len)
        == 0) {
      RxFree(re);
      return 0;
     }
...
int EBuffer::ExpandTemplate(ExState& State) {
  ...
  if (State.GetStrParam(View, FileName, MAXPATH) == 0) {
    if (View->MView->Win->GetStr("Filename", MAXPATH, FileName,
                                 HIST_PATH) == 0) return 0;
  }
  ...
  while (RxExec(re, buf, len, buf, &match) && match.Open[2] != -1) {
    ...
    if (View->MView->Win->GetStr(prompts[num], sizeof(values[num]), values[num],
                                 HIST_DEFAULT) == 0) {
      RxFree(re);
      return 0;
    }
...
int EBuffer::MoveToLine(ExState& State) {
  ...
  if (State.GetIntParam(View, &No) == 0) {
    ...
    if (View->MView->Win->GetStr("Goto Line", sizeof(Num), Num,
                                 HIST_POSITION) == 0) return 0;
...
int EBuffer::MoveToColumn(ExState& State) {
  ...
  if (State.GetIntParam(View, &No) == 0) {
    ...
    if (View->MView->Win->GetStr("Goto Column", 8, Num,
                                 HIST_POSITION) == 0) return 0;
...
int EBuffer::FoldCreateByRegexp(ExState& State) {
  ...
  if (State.GetStrParam(View, strbuf, sizeof(strbuf)) == 0) {
    if (View->MView->Win->GetStr("Create Fold Regexp", sizeof(strbuf), strbuf,
                                 HIST_REGEXP) == 0) return 0;
  }
...
int EBuffer::PlaceUserBookmark(const char *n, EPoint P) {
  ...
  if (result) {
    ...
    if ((BFI(this,
             BFI_SaveBookmarks) == 1) || (BFI(this, BFI_SaveBookmarks) == 2)) {
...
int EBuffer::GetUserBookmarkForLine(int     searchFrom,
                                    int     searchForLine,
                                    char *& Name,
                                    EPoint& P) {
  int i;
...
int EBuffer::PlaceBookmark(ExState& State) {
  ...
  if (State.GetStrParam(View, name,
                        sizeof(name)) ==
      0) if (View->MView->Win->GetStr("Place Bookmark", sizeof(name), name,
...
int EBuffer::RemoveBookmark(ExState& State) {
  ...
  if (State.GetStrParam(View, name,
                        sizeof(name)) ==
      0) if (View->MView->Win->GetStr("Remove Bookmark", sizeof(name), name,
                                      HIST_BOOKMARK) == 0) return 0;
...
int EBuffer::GotoBookmark(ExState& State) {
  ...
  if (State.GetStrParam(View, name,
                        sizeof(name)) ==
      0) if (View->MView->Win->GetStr("Goto Bookmark", sizeof(name), name,
                                      HIST_BOOKMARK) == 0) return 0;
...
int EBuffer::PlaceGlobalBookmark(ExState& State) {
  ...
  if (State.GetStrParam(View, name,
                        sizeof(name)) ==
      0) if (View->MView->Win->GetStr("Place Global Bookmark", sizeof(name), name,
                                      HIST_BOOKMARK) == 0) return 0;
...
int EBuffer::PushGlobalBookmark() {
  ...
  if (m) Msg(S_INFO, "Placed bookmark %s", m->GetName());
  return m ? 1 : 0;
}
...
int EBuffer::InsertString(ExState& State) {
  ...
  if (State.GetStrParam(View, strbuf, sizeof(strbuf)) == 0) {
    if (View->MView->Win->GetStr("Insert String", sizeof(strbuf), strbuf,
...
int EBuffer::SelfInsert(ExState& /*State*/) {
  if (LastEventChar != -1) return TypeChar(char(LastEventChar));
...
int EBuffer::FileSaveAs(const char *FName) {
  ...
  if (ExpandPath(FName, Name, sizeof(Name)) == -1) {
    View->MView->Win->Choice(GPC_ERROR,
                             "Error",
                             1,
                             "O&K",
                             "Invalid path: %s.",
...
int EBuffer::FileSaveAs(ExState& State) {
  ...
  if (State.GetStrParam(View, FName, sizeof(FName)) == 0)
    if (View->MView->Win->GetFile("Save As", sizeof(FName), FName, HIST_PATH,
                                  GF_SAVEAS) == 0) return 0;
...
int EBuffer::FileWriteTo(ExState& State) {
  ...
  if (State.GetStrParam(View, FName,
                        sizeof(FName)) ==
      0) if (View->MView->Win->GetFile("Write To", sizeof(FName), FName,
                                       HIST_PATH,
                                       GF_SAVEAS) == 0) return 0;
...
int EBuffer::BlockReadX(ExState& State, int blockMode) {
  ...
  if (State.GetStrParam(View, FName,
                        sizeof(FName)) ==
      0) if (View->MView->Win->GetFile("Read block", sizeof(FName), FName,
                                       HIST_PATH,
                                       GF_OPEN) == 0) return 0;
...
int EBuffer::BlockWrite(ExState& State) {
  ...
  if (State.GetStrParam(View, FName, sizeof(FName)) == 0)
    if (View->MView->Win->GetFile("Write block", sizeof(FName), FName, HIST_PATH,
                                  GF_SAVEAS) == 0) return 0;
...
int EBuffer::Find(ExState& State) {
  ...
  if (State.GetStrParam(View, find, sizeof(find)) != 0) {
    if (State.GetStrParam(View, options, sizeof(options)) == 0) strcpy(options,
                                                                       BFS(this,
                                                                           BFS_DefFindOpt));
    ...
    if (BFS(this, BFS_DefFindOpt)) strcpy(options, BFS(this, BFS_DefFindOpt));

    if (ParseSearchOptions(0, options, LSearch.Options) == 0) LSearch.Options = 0;

    if (DLGGetFind(View->MView->Win, LSearch) == 0) return 0;
  } else {
    if (BFS(this, BFS_DefFindOpt)) strcpy(options, BFS(this, BFS_DefFindOpt));

    if (View->MView->Win->GetStr("Find", sizeof(find), find,
                                 HIST_SEARCH) == 0) return 0;

    if (View->MView->Win->GetStr(
          "Options (All/Block/Cur/Delln/Glob/Igncase/Joinln/Rev/Word/regX)",
          sizeof(options), options, HIST_SEARCHOPT) == 0) return 0;
...
int EBuffer::FindReplace(ExState& State) {
  ...
  if (State.GetStrParam(View, find, sizeof(find)) != 0) {
    if (State.GetStrParam(View, replace, sizeof(replace)) == 0) return 0;

    if (State.GetStrParam(View, options, sizeof(options)) == 0) return 0;
    ...
    if (BFS(this, BFS_DefFindReplaceOpt)) strcpy(options,
                                                 BFS(this, BFS_DefFindReplaceOpt));

    if (ParseSearchOptions(1, options, LSearch.Options) == 0) LSearch.Options = 0;

    if (DLGGetFindReplace(View->MView->Win, LSearch) == 0) return 0;
  } else {
    if (BFS(this, BFS_DefFindReplaceOpt)) strcpy(options,
                                                 BFS(this, BFS_DefFindReplaceOpt));

    if (State.GetStrParam(View, find,
                          sizeof(find)) ==
        0) if (View->MView->Win->GetStr("Find", sizeof(find), find,
                                        HIST_SEARCH) == 0) return 0;

    if (State.GetStrParam(View, replace,
                          sizeof(replace)) ==
        0) if (View->MView->Win->GetStr("Replace", sizeof(replace), replace,
                                        HIST_SEARCH) == 0) return 0;

    if (State.GetStrParam(View, options,
                          sizeof(options)) ==
        0) if (View->MView->Win->GetStr(
                 "Options (All/Block/Cur/Delln/Glob/Igncase/Joinln/Rev/Splitln/Noask/Word/regX)",
                 sizeof(options), options,
                 HIST_SEARCHOPT) == 0) return 0;
...
int EBuffer::ChangeMode(ExState& State) {
  ...
  if (State.GetStrParam(View, Mode,
                        sizeof(Mode)) ==
      0) if (View->MView->Win->GetStr("Mode", sizeof(Mode), Mode,
...
int EBuffer::ChangeTabSize(ExState& State) {
  ...
  if (State.GetIntParam(View, &No) == 0) {
    ...
    if (View->MView->Win->GetStr("TabSize", sizeof(Num), Num,
                                 HIST_SETUP) == 0) return 0;
...
int EBuffer::ChangeRightMargin(ExState& State) {
  ...
  if (State.GetIntParam(View, &No) == 0) {
    ...
    if (View->MView->Win->GetStr("RightMargin", sizeof(Num), Num,
                                 HIST_SETUP) == 0) return 0;
...
int EBuffer::ChangeLeftMargin(ExState& State) {
  ...
  if (State.GetIntParam(View, &No) == 0) {
    ...
    if (View->MView->Win->GetStr("LeftMargin", sizeof(Num), Num,
                                 HIST_SETUP) == 0) return 0;
  ...
  if (Modified) {
    if (multiFile) {
      ...
      case 1: { /* As */
        ...
        if (V->GetFile("Save As", sizeof(FName), FName, HIST_PATH,
                       GF_SAVEAS) == 0) return 0;
...
void EBuffer::GetTitle(char *ATitle, int MaxLen, char *ASTitle,
                       int SMaxLen) const {
  char *p;
...
int EBuffer::ASCIITable(ExState& /*State*/) {
  ...
  if (rc != -1) return InsertChar(char(rc));
...
int EBuffer::ScrollLeft(ExState& State) {
  ...
  if (State.GetIntParam(View, &Cols) == 0) Cols = 8;
...
int EBuffer::ScrollDown(ExState& State) {
  ...
  if (State.GetIntParam(View, &Rows) == 0) Rows = 1;
...
int EBuffer::FindTag(ExState& State) {
  ...
  if (State.GetStrParam(View, Tag,
                        sizeof(Tag)) ==
      0) if (View->MView->Win->GetStr("Find tag", sizeof(Tag), Tag,
                                      HIST_SEARCH) == 0) return 0;
  ...
  while (j--) {
    ...
    if (i > 0) return 1;
      ...
      /* Try autoload tags */
      if (View->ExecCommand(ExTagLoad, State) == 0) break;
    } else {
...
int EBuffer::ShowHelpWord(ExState& State) {
  ...
  for (int i = 0; i < P; i++) {
    // printf("%d - %d  %d   %c %c\n", i, P, L->Chars[i],
    // L->Chars[i], L->Chars[P]);
    if ((L->Chars[i] == '\b') && (P < (L->Count - 2))) P += 2;
  }
  size_t len = 0;
  ...
  if (P < L->Count) {
    ...
    while ((P > 0)
           && ((L->Chars[P - 1] == '\b') || isalnum(L->Chars[P - 1])
               || (strchr(achr, L->Chars[P - 1]) != NULL))) P--;  // '_' for
                                                                  // underline
                                                                  // is hidden
                                                                  // in achr

    if ((P < (L->Count - 1)) && (L->Chars[P] == '\b')) P++;

    // To end of word,
    while ((len < (sizeof(buf) - 1)) && (P < L->Count)) {
      if (((P + 1) < L->Count) && (L->Chars[P + 1] == '\b')) P += 2;
      else if (isalnum(L->Chars[P])
               || (strchr(achr, L->Chars[P]) != NULL)) buf[len++] = L->Chars[P++];
      else break;
    }
  }
  buf[len] = 0;
...
int EBuffer::GetStrVar(int var, char *str, int buflen) {
  ...
  case mvFileBaseName: {
    ...
    while ((dot2 = strchr(dot + 1, '.')) != NULL) dot = dot2;
