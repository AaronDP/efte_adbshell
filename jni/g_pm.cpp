...
public:
  ...
  int  QuerySbVPos();
  int  SetSbVPos(int Start,
                 int Amount,
                 int Total);
  int  SetSbHPos(int Start,
                 int Amount,
                 int Total);
...
HWND CreatePMMainMenu(HWND  parent,
                      HWND  owner,
                      char *Name);
HWND CreatePMMenu(HWND parent,
                  HWND owner,
                  int  menu,
                  int  id,
                  int  style);
...
MRESULT EXPENTRY AVIOWndProc(HWND   hwnd,
                             ULONG  msg,
                             MPARAM mp1,
                             MPARAM mp2);
MRESULT EXPENTRY ObjectWndProc(HWND   hwnd,
                               ULONG  msg,
                               MPARAM mp1,
                               MPARAM mp2);
MRESULT EXPENTRY CreatorWndProc(HWND   hwnd,
                                ULONG  msg,
                                MPARAM mp1,
                                MPARAM mp2);
MRESULT EXPENTRY AboutDlgProc(HWND   hwnd,
                              ...
                              MPARAM mp1,
                              MPARAM mp2);
MRESULT EXPENTRY InfoDlgProc(HWND   hwnd,
                             ULONG  msg,
                             MPARAM mp1,
                             MPARAM mp2);
...
HWND CreateToolBar(HWND parent, HWND owner, int id) {
  ...
  for (i = 0; i < sizeof(tools) / sizeof(tools[0]); i++) {
    if ((tools[i].hBitmap == 0) &&
        (tools[i].ulType == tiBITMAP)) tools[i].hBitmap = GpiLoadBitmap(
        hps,
        NULLHANDLE,
        tools[i].
        ulId,
...
void InsertHistory(HWND hwnd, int id, int maxlen) {
  ...
  for (i = 0; i < count; i++) {
    if (GetInputHistory(id, str, maxlen, i + 1) == 1) WinInsertLboxItem(hwnd,
                                                                        LIT_END,
                                                                        str);
  }
  free(str);
...
MRESULT EXPENTRY FileDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
  ...
  case WM_INITDLG:
    ...
    WinRestoreWindowPos("eFTEPM",
                        ((dlg->fl &
                          FDS_SAVEAS_DIALOG) ? "FileSaveDlg" : "FileOpenDlg"),
                        hwnd);
    break;
...
static int DoChoice(HWND hwndFrame, ChoiceInfo *choice)
{
  ...
  for (int i = 0; i < choice->NSel; i++) {
    ...
    hwndButton[i] =
      WinCreateWindow(hwndDlg,
                      ...
                      ((i == 0) ? BS_DEFAULT | WS_TABSTOP | WS_GROUP : 0),
  ...
  HWND hwndStatic = WinCreateWindow(
    hwndDlg,
    WC_STATIC,
    Prompt,
    WS_VISIBLE | SS_TEXT | DT_TOP | DT_CENTER |
    DT_WORDBREAK,
    0,
    0,
    0,
...
/* reimplemented most of the WinMessageBox code to get store/restore position to
  work */
int DLGPickChoice(GView      *View,
                  const char *ATitle,
                  int         NSel,
                  va_list     ap,
                  int         Flags) {
  ...
  rc =
    LONGFROMMR(WinSendMsg(View->Parent->Peer->hwndFrame, UWM_CHOICE,
                          MPFROMP(&choice),
                          0));
...
MRESULT EXPENTRY PromptDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
  ...
  case WM_INITDLG:
    ...
    InsertHistory(WinWindowFromID(hwnd,
                                  IDE_FIELD), PromptInfo.HistId,
                  PromptInfo.MaxLen);
...
int DLGGetStr(GView       *View,
  ...
  if (LONGFROMMR(WinSendMsg(View->Parent->Peer->hwndFrame,
                            ...
                            MPFROMLONG(IDD_PROMPT))) != DID_OK) return 0;
...
MRESULT EXPENTRY FindDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
  ...
  case WM_INITDLG:
    ...
    if (ReplaceDlg) {
      WinSendDlgItemMsg(hwnd, IDE_REPLACE, EM_SETTEXTLIMIT, MPFROMLONG(
                          MAXSEARCH), 0);
  ...
  case WM_COMMAND:
    ...
    case DID_OK:
      ...
      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_IGNORECASE)) SearchOpt.Options |=
          SEARCH_NCASE;

      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_REGEXPS)) SearchOpt.Options |= SEARCH_RE;

      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_WORDS)) SearchOpt.Options |= SEARCH_WORD;

      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_BLOCK)) SearchOpt.Options |= SEARCH_BLOCK;

      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_GLOBAL)) SearchOpt.Options |=
          SEARCH_GLOBAL;

      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_REVERSE)) SearchOpt.Options |= SEARCH_BACK;

      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_ALLOCCURENCES)) SearchOpt.Options |=
          SEARCH_ALL;

      if (WinQueryButtonCheckstate(hwnd,
                                   IDC_JOINLINE)) SearchOpt.Options |=
          SEARCH_JOIN;
      ...
      if (ReplaceDlg) {
        ...
        if (WinQueryButtonCheckstate(hwnd,
                                     IDC_NOPROMPTING)) SearchOpt.Options |=
            SEARCH_NASK;
      } else {
        if (WinQueryButtonCheckstate(hwnd,
                                     IDC_DELETELINE)) SearchOpt.Options |=
            SEARCH_DELETE;
      }
...
int DLGGetFind(GView *View, SearchReplaceOptions& sr) {
  ...
  if (LONGFROMMR(WinSendMsg(View->Parent->Peer->hwndFrame, UWM_DLGBOX,
                            ...
                            MPFROMLONG(IDD_FIND))) != DID_OK) return 0;
...
int DLGGetFindReplace(GView *View, SearchReplaceOptions& sr) {
  ...
  if (LONGFROMMR(WinSendMsg(View->Parent->Peer->hwndFrame, UWM_DLGBOX,
                            PVOIDFROMMP(PFNWP(FindDlgProc)),
                            MPFROMLONG(IDD_FINDREPLACE))) != DID_OK) return 0;
...
char* ConvertKey(int ch, int virt, int flags, int scan, TEvent& Event) {
  ...
  if (flags & KC_CTRL) keyFlags |= kfCtrl;

  if (flags & KC_ALT) keyFlags |= kfAlt;

  if (flags & KC_SHIFT) keyFlags |= kfShift;

  if ((ch != 0xE0) && ((ch & 0xFF) == 0xE0)) keyFlags |= kfGray;
  ...
  if (keyCode == 0) {
    ...
    if ((ch == '+') && (scan == 78)) keyCode = '+' | kfGray;
    else if ((ch == '-') && (scan == 74)) keyCode = '-' | kfGray;
    else if ((ch == '*') && (scan == 55)) keyCode = '*' | kfGray;
    else if ((ch == '/') && (scan == 92)) keyCode = '/' | kfGray;
    else {
      keyCode = ch;
    ...
    if (keyCode & kfGray) strcpy(keyname, "G+");
  ...
  if ((keyFlags & (kfCtrl | kfAlt | kfSpecial | kfGray)) == kfCtrl) {
    if ((keyCode >= 'a') && (keyCode < 'a' + 32)) keyCode -= 'a' - 1;
    else if ((keyCode >= 'A') && (keyCode < 'A' + 32)) keyCode -= 'A' - 1;
  }
  ...
  if (keyCode & kfKeyUp) strcat(name, "UP ");
  else strcat(name, "DN ");

  if (keyCode & kfAlt) strcat(name, "A+");

  if (keyCode & kfCtrl) strcat(name, "C+");

  if (keyCode & kfGray) strcat(name, "G+");

  if (keyCode & kfShift) strcat(name, "S+");
    strcat(name, keyname);
  ...
  if (peer) switch (msg) {
    ...
    case UWM_DLGBOX:
      return MRFROMLONG(WinDlgBox(HWND_DESKTOP, hwnd,
                                  PFNWP(PVOIDFROMMP(mp1)), 0, LONGFROMMP(mp2),
                                  NULL));
    ...
    case UWM_CREATECHILD:
      ...
      // DosBeep(2500, 1000);
      return CreateChild(hwnd, (GViewPeer *)PVOIDFROMMP(
                           mp1), (PMData *)PVOIDFROMMP(mp2));
      ...
      // check for keys not to be translated
    {
      ...
      if (((vk == VK_MENU) || (vk == VK_F1)) && (fl == 0) ||
          ...
          (vk == VK_SPACE) && (fl == KC_ALT)) return (MRESULT)FALSE;

      if (((ch >= 'A') && (ch <= 'Z') || (ch >= 'a') && (ch <= 'z')) &&
          (fl & KC_ALT)) return (MRESULT)FALSE;

      if (PMDisableAccel)
        if (fl & KC_ALT)
          if ((vk >= VK_F1) && (vk <= VK_F24)) return (MRESULT)FALSE;
    ...
    case WM_MINMAXFRAME: {
      ...
      if (pswp->fl & SWP_MAXIMIZE) {
        ...
        if (v) {
          ...
          if (pswp->cx > MAXXSIZE) pswp->cx = MAXXSIZE;
          pswp->cx *= v->Peer->pmData->cxChar;

          pswp->cy /= v->Peer->pmData->cyChar;

          if (pswp->cy > MAXYSIZE) pswp->cy = MAXYSIZE;
          pswp->cy *= v->Peer->pmData->cyChar;
        }
    ...
    case WM_ADJUSTWINDOWPOS: {
      ...
      if (pswp->fl & (SWP_SIZE | SWP_MOVE | SWP_MAXIMIZE)) {
        ...
        if ((pswp->cx < 0) || (pswp->cy <= cyTitleBar + 2 * cyBorder)) break;

        DosRequestMutexSem(hmtxPMData, SEM_INDEFINITE_WAIT);

        // printf("Before 1: %d %d | %d %d\n", pswp->cx, pswp->x, pswp->cy,
        // pswp->y);
    ...
    case WM_FORMATFRAME: {
      ...
      if (ShowToolBar) {
        ...
        if (ShowToolBar == 1) {
          ...
          if (prcl) prcl->yTop -= swpToolBar.cy;
          ...
          if (prcl) prcl->yBottom += swpToolBar.cy;
        }
        sCount++;
      }
...
MRESULT EXPENTRY AVIOWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
  ...
  case WM_CREATE:
    ...
    if (WindowFont[0] != 0) {
      ...
      if (sscanf(WindowFont, "%dx%d", &x,
                 &y) == 2) VioSetDeviceCellSize((SHORT)y, (SHORT)x, pmData->hvps);
    }
  ...
  case WM_MOUSEMOVE: {
    ...
    if (WinGetKeyState(HWND_DESKTOP, VK_BUTTON1) & 0x8000) b |= 1;

    if (WinGetKeyState(HWND_DESKTOP, VK_BUTTON2) & 0x8000) b |= 2;

    if (WinGetKeyState(HWND_DESKTOP, VK_BUTTON3) & 0x8000) b |= 4;
  ...
  case WM_PAINT:
    DosRequestMutexSem(hmtxPMData, SEM_INDEFINITE_WAIT);
    {
      ...
      if ((Peer->wH > 0) && (Peer->wH <= MAXYSIZE) &&
          (Peer->wW > 0) &&
          (Peer->wW <= MAXXSIZE)) VioShowBuf(0,
                                             (SHORT)(MAXXSIZE * Peer->wH * 2),
                                             pmData->hvps);
  ...
  case DM_DRAGOVER: {
    ...
    if ((pDragInfo->cditem > 1) ||
        (strstr(buf,
                "DRM_OS2FILE") == 0)) return MRFROM2SHORT((DOR_NEVERDROP),
                                                          (DO_UNKNOWN));
...
MRESULT EXPENTRY CreatorWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
  ...
  case UWM_CREATEFRAME: {
    ...
    if (ShowToolBar) {
      peer->hwndToolBar = CreateToolBar(peer->hwndFrame,
                                        peer->hwndFrame,
                                        FID_MTOOLBAR);
  ...
  case UWM_CREATEMAINMENU: {
    ...
    if (len > 0) WinSetPresParam(hwndMenu, AttrFound, len, font);
  ...
  case UWM_CREATEPOPUPMENU: {
    ...
    len = WinQueryPresParam(peer->menuBar,
                            PP_FONTNAMESIZE,
                            PP_FONTHANDLE,
                            &AttrFound,
                            sizeof(font),
    ...
    if (len > 0) WinSetPresParam(hwnd, AttrFound, len, font);
...
int ConGetEvent(TEventMask EventMask,
  ...
  if (WaitTime != -1) {
    if (WinPeekMsg(hab, &qmsg, NULLHANDLE, 0, 0, PM_NOREMOVE) == FALSE) return 0;
  }

  if (WinGetMsg(hab, &qmsg, NULLHANDLE, 0, 0) == FALSE) return -1;
      ...
      if (v->Peer->hwndWorker == qmsg.hwnd) {
        ...
        case WM_SETFOCUS:
          ...
          if (SHORT1FROMMP(qmsg.mp2) == TRUE) {
            if (!v->IsActive()) v->Parent->SelectView(v);
        ...
        case WM_VSCROLL:
          ...
          case SB_SLIDERTRACK: {
            ...
            if (x != 0) {
              ...
              if (v->Peer->sbVtotal > 32000) Event->Msg.Param1 =
                  (x - 1) * v->Peer->sbVtotal / 32000;
              else Event->Msg.Param1 = x - 1;
            }
            break;
          }
          }
        ...
        case WM_HSCROLL:
          ...
          case SB_SLIDERTRACK: {
            ...
            if (x != 0) {
              ...
              if (v->Peer->sbHtotal > 32000) Event->Msg.Param1 =
        ...
        case WM_MOUSEMOVE:
          ...
          Event->Msg.View   = v;
          Event->Mouse.X    = ((SHORT)SHORT1FROMMP(qmsg.mp1)) /
                              v->Peer->pmData->cxChar;
          Event->Mouse.Y = v->Peer->wH - ((SHORT)SHORT2FROMMP(qmsg.mp1)) /
                           v->Peer->pmData->cyChar - 1;
          Event->Mouse.Buttons = 1;
          Event->Mouse.Count   = 1;
          Event->Mouse.KeyMask = 0;

          if (WinGetKeyState(HWND_DESKTOP,
                             VK_CTRL) & 0x8000) Event->Mouse.KeyMask |= kfCtrl;

          if (WinGetKeyState(HWND_DESKTOP,
                             VK_ALT) & 0x8000) Event->Mouse.KeyMask |= kfAlt;

          if (WinGetKeyState(HWND_DESKTOP,
                             VK_SHIFT) & 0x8000) Event->Mouse.KeyMask |= kfShift;
          ...
          case WM_MOUSEMOVE:
            ...
            if (WinGetKeyState(HWND_DESKTOP,
                               VK_BUTTON1) & 0x8000) Event->Mouse.Buttons |= 1;

            if (WinGetKeyState(HWND_DESKTOP,
                               VK_BUTTON2) & 0x8000) Event->Mouse.Buttons |= 2;

            if (WinGetKeyState(HWND_DESKTOP,
                               VK_BUTTON3) & 0x8000) Event->Mouse.Buttons |= 4;
...
GViewPeer::GViewPeer(GView *view,
                     int    XSize,
                     int    YSize) {
  HWND parent;
...
int GViewPeer::ConScroll(int   Way,
                         int   X,
                         int   Y,
                         int   W,
                         int   H,
  ...
  case csUp:
    VioScrollUp((USHORT)Y, (USHORT)X, (USHORT)(Y + H - 1), (USHORT)(X + W - 1),
                (USHORT)Count, (PBYTE)&FillCell, pmData->hvps);
    break;
...
int GViewPeer::ConSetCursorPos(int X, int Y) {
  ...
  if (wState & sfFocus) return PMSetCursorPos();
  else return 1;
}
...
int GViewPeer::ConShowCursor() {
  ...
  if (wState & sfFocus) return PMShowCursor();
  else return 1;
}
...
int GViewPeer::ConHideCursor() {
  ...
  if (wState & sfFocus) return PMHideCursor();
  else return 1;
}
...
int GViewPeer::ExpandHeight(int DeltaY) {
  if (View->Parent->Top == View->Next) return -1;

  if (DeltaY + wH < 3) return -1;

  if (View->Next->Peer->wH - DeltaY < 3) return -1;
...
int GViewPeer::UpdateCursor() {
  ...
  if (cVisible) ConShowCursor();
  else ConHideCursor();
  return 1;
}

int GViewPeer::PMShowCursor() {
  if (wState & sfFocus) WinShowCursor(hwndView, TRUE);
  return 1;
}

int GViewPeer::PMHideCursor() {
  if (wState & sfFocus) WinShowCursor(hwndView, FALSE);
  return 1;
}
...
GFramePeer::GFramePeer(GFrame *aFrame,
                       int     Width,
                       int     Height) {
  Frame = aFrame;
  ...
  if ((Width != -1) && (Height != -1)) ConSetSize(Width, Height);
}
...
int GFrame::SetMenu(const char *Name) {
  ...
  if (Menu && Name && (strcmp(Name, Menu) == 0)) return 1;
  ...
  if (Sub != 0) {
    ...
    for (SHORT i = 0; i < count; i++) {
      ...
      if (id == MIT_ERROR) {
        ...
        // printf("got %d %d\n", i, id);
        len =
          SHORT1FROMMR(WinSendMsg(hwnd, MM_QUERYITEMTEXT,
                                  MPFROM2SHORT((id),
                                               (sizeof(buf) - 1)), MPFROMP(buf)));
        buf[len] = 0;
    ...
    WinPostMsg(hwnd, MM_STARTMENUMODE, MPFROM2SHORT((Sub ? TRUE : FALSE),
                                                    FALSE), 0);
  }
  return 0;
}
...
int GUI::ConGetEvent(TEventMask EventMask,
                     TEvent    *Event,
                     int        WaitTime,
                     int        Delete,
                     GView    **view) {
...
int GUI::Run() {
  ...
  while (doLoop != 0 && WinGetMsg(hab, &qmsg, NULLHANDLE, 0, 0)) WinDispatchMsg(
      hab,
      &qmsg);
  return 0;
}
...
int GUI::RunProgram(int mode, char *Command) {
...

  {
    ...
    sd.IconFile      = 0;
    sd.PgmHandle     = 0;
    sd.PgmControl    = SSF_CONTROL_VISIBLE; // | ((Command && Command[0] != 0) ?
                                            // SSF_CONTROL_NOAUTOCLOSE : 0);
...
static void _LNK_CONV PipeThread(void *p) {
  ...
  if (rc != 0) {
    ...
    if (pipe->notify) WinPostMsg(frames->Active->Peer->hwndWorker,
                                 UWM_NOTIFY,
                                 MPFROMLONG(pipe->notify),
                                 MPFROMLONG(pipe->id));
...
void DieError(int rc, const char *msg, ...) {
  ...
  if (hab == 0) hab = WinInitialize(0);

  if (hmq == 0) hmq = WinCreateMsgQueue(hab, 0);
