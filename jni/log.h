...
# include <string.h>
# include <stdlib.h>
#else // if defined(NO_NEW_CPP_FEATURES)
# include <fstream>
# include <cassert>
# include <cstring>
# include <cstdlib>
#endif // if defined(NO_NEW_CPP_FEATURES)
...
public:
  ...
  GlobalLog(char const *strLogFile) : m_strLogFile(strdup(strLogFile)),
                                      m_bOpened(false) {}
...
# define ENDFUNCRC(rc) \
  do { LOGOBJNAME.RC(__LINE__) << (rc) << ENDLINE; return (rc); } while (0)
# define ENDFUNCRC_SAFE(type, rc)                                          \
  do { type LOG__RC = (rc); LOGOBJNAME.RC(__LINE__) << LOG__RC << ENDLINE; \
       return LOG__RC; } while (0)
# define ENDFUNCAS(type, rc) \
  do { LOGOBJNAME.RC(__LINE__) << (type)(rc) << ENDLINE; return (rc); } while (0)
# define ENDFUNCAS_SAFE(logtype, rctype, rc)                                 \
  do { rctype LOG__RC = (rc); LOGOBJNAME.RC(__LINE__) << (logtype)LOG__RC << \
       ENDLINE; return LOG__RC; } while (0)
...
private:                                                                        \
    ...
    type1 const& o1;                                                            \
public:                                                                         \
    ostream_func1_ ## type1(ostream & (*osfunc_)(ostream &, type1 const &),     \
                            type1 const & o1_) : osfunc(osfunc_), o1(o1_) {}    \
...
private:                                                                          \
    ...
    type2 const& o2;                                                              \
public:                                                                           \
    ostream_func2_ ## type1 ## _ ## type2(ostream &                               \
                                          (*osfunc_)(ostream &, type1 const &,    \
                                                     type2 const &),              \
                                          type1 const & o1_, type2 const & o2_) : \
      osfunc(osfunc_), o1(o1_), o2(o2_) {}                                        \
    ostream& operator()(ostream& os) const {                                      \
      return osfunc(os, o1, o2);                                                  \
    }                                                                             \
  };                                                                              \
  inline ostream& operator<<(ostream                                    & os,     \
                             ostream_func2_ ## type1 ## _ ## type2 const& ofunc)  \
  {                                                                               \
    return ofunc(os);                                                             \
  }
...
ostream&                         Log__osFillChar(ostream&,
                                                 char const&,
                                                 size_t const&);
...
void Log__BinaryData(FunctionLog&,
                     void         *bin_data,
                     size_t        len,
                     unsigned long line);
# define LOGBINARYDATA(bin_data, len) Log__BinaryData(LOGOBJNAME, \
                                                      bin_data,   \
                                                      len,        \
                                                      __LINE__)
