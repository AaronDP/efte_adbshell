...
static int copyfile(const char *f1, const char *f2) { // from F1 to F2
  ...
  if ((fd1 = open(f1, O_RDONLY | O_BINARY)) == -1) return -1;
...
char* MakeBackup(const char *FileName, char *NewName) {
  ...
  if (!IsSameFile(FileName, NewName)) {
    ...
    if (copyfile(FileName, NewName) == 0) return NewName;

#if 0

    if (errno == 2) return NewName;  /* file not found */
...
int GetCharFromEvent(TEvent& E, char *Ch) {
  *Ch = 0;
  ...
  if (E.Key.Code & kfModifier) return 0;
...
int CompletePath(const char *Base, char *Match, int Count) {
  ...
  while (rc == 0) {
    ...
    if ((strcmp(dname, ".") != 0) &&
        (strcmp(dname, "..") != 0) &&
        (!CompletionFilter ||
         (RxExec(CompletionFilter, dname, strlen(dname), dname, &RM) != 1))) {
      if ((
#if defined(UNIX)
            strncmp
#else // os2, nt, ...
        ...
        if (Count == count) {
          ...
          if (!hascname) {
            ...
            while (cname[o] && dname[o] &&
                   (toupper(cname[o]) == toupper(dname[o]))) o++;
