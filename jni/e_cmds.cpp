...
int EBuffer::MoveLeft() {
  if (CP.Col == 0) {
    if ((CursorWithinEOL == 1) && MoveUp()) return MoveLineEnd();
    else return 0;
  }
...
int EBuffer::MoveRight() {
  if ((CursorWithinEOL == 1) && (CP.Col == LineLen())) {
    if (MoveDown()) return MoveLineStart();
    else return 0;
  }
...
int EBuffer::MoveUp() {
  if (LastUpDownColumn == -1) LastUpDownColumn = CP.Col;
  ...
  if (CursorWithinEOL == 1) {
    ...
    if (CP.Col > LastUpDownColumn) SetPos(LastUpDownColumn, CP.Row);
  }
...
int EBuffer::MoveNext() {
  if (CP.Col < LineLen()) if (MoveRight()) return 1;
...
int EBuffer::MoveWordOrCapLeft() {
  if (CP.Col > 0) {
    ...
    if (P > 0) {
      ...
      while ((P > 0) &&
             (WGETBIT(Flags.WordChars,
                      L->Chars[P - 1]) == 1) &&
             (WGETBIT(Flags.CapitalChars, L->Chars[P - 1]) == 0)) P--;
...
int EBuffer::MoveWordOrCapRight() {
  ...
  while ((P < L->Count) &&
         (WGETBIT(Flags.WordChars,
                  L->Chars[P]) == 1) &&
         (WGETBIT(Flags.CapitalChars, L->Chars[P]) == 0)) P++;
...
int EBuffer::MoveBlockStart() {
  if ((BB.Col == -1) && (BB.Row == -1)) return 0;

  assert(BB.Col >= 0 && BB.Row >= 0 && BB.Row < RCount);

  if (SetPosR(BB.Col, BB.Row)) return 1;

  return 0;
}

int EBuffer::MoveBlockEnd() {
  if ((BE.Col == -1) && (BE.Row == -1)) return 0;

  assert(BE.Col >= 0 && BE.Row >= 0 && BE.Row < RCount);

  if (SetPosR(BE.Col, BE.Row)) return 1;
...
int EBuffer::MovePrevEqualIndent() {
  ...
  while (--L >= 0)
    if ((RLine(L)->Count > 0) && (LineIndented(L) == I)) return SetPosR(I, L);
...
int EBuffer::MoveLineTop() {
  if (View) if (GetVPort()->SetTop(GetVPort()->TP.Col, CP.Row) == 0) return 0;
...
int EBuffer::MoveTabStart() {
  ...
  if (C < X->Count)
    if (X->Chars[C] == 9) return SetPos(ScreenPos(X, C), CP.Row);
...
int EBuffer::MoveTabEnd() {
  ...
  if (C < X->Count)
    if (X->Chars[C] == 9)
      if (ScreenPos(X, C) < CP.Col) return SetPos(ScreenPos(X, C + 1), CP.Row);
...
int EBuffer::MoveBeginOrNonWhite() {
  if (CP.Col == 0) return MoveFirstNonWhite();
  else return MoveLineStart();
}

int EBuffer::MoveBeginLinePageFile() {
  int L = GetVPort()->TP.Row;

  if ((CP.Col == 0) && (CP.Row == L)) return MoveFileStart();
  else if (CP.Col == 0) return MovePageStart();
  else return MoveLineStart();
}
...
int EBuffer::MoveEndLinePageFile() {
  ...
  if ((CP.Col == Len) && (CP.Row == L)) return MoveFileEnd();
  else if (CP.Col == Len)
    if (MovePageEnd() == 0) return 0;
...
int EBuffer::KillCharPrev() {
  if (CP.Col == 0) {
    if (CP.Row > 0) if (ExposeRow(VToR(CP.Row) - 1) == 0) return 0;
...
int EBuffer::KillWordOrCap() {
  ...
  if (CP.Col >= LineLen()) {
    ...
    if (Class == 1) {
      if (WGETBIT(Flags.CapitalChars,
                  L->Chars[P]) ==
          1) while ((P < L->Count) &&
                    (WGETBIT(Flags.CapitalChars, L->Chars[P]) == 1)) P++;
...
int EBuffer::KillBlockOrChar() {
  if (CheckBlock() == 0) return KillChar();
  else return BlockKill();
}

int EBuffer::KillBlockOrCharPrev() {
  if (CheckBlock() == 0) return KillCharPrev();
  else return BlockKill();
}
...
int EBuffer::BackSpace() {
  ...
  if ((CheckBlock() == 1) && BFI(this, BFI_BackSpKillBlock)) {
    if (BlockKill() == 0) return 0;
                  ...
                  BFI_WordWrap) == 2) && (CP.Row > 0) && !IsLineBlank(Y - 1) &&
             (CP.Col <=
              BFI(this, BFI_LeftMargin)) && (CP.Col <= LineIndented(Y))) {
    ...
    if (BFI(this, BFI_BackSpUnindents) && (LineIndented(Y) == CP.Col)) {
      ...
      while (L > 0 &&
             (IsLineBlank(L - 1) || (C1 = LineIndented(L - 1)) >= C)) L--;
      ...
      if (BFI(this, BFI_Insert) == 0) if (InsText(Y, C1, 1, " ") == 0) return 0;
      ...
      if (BFI(this,
              BFI_Insert) ==
          0) if (InsText(Y, ScreenPos(L, C), 1, " ") == 0) return 0;
    }
  }
...
int EBuffer::LineSplit() {
  ...
  if (BFI(this, BFI_Trim)) if (TrimLine(VToR(CP.Row)) == 0) return 0;
...
int EBuffer::TypeChar(char aCh) { // does abbrev expansion if appropriate
  ...
  if ((ChClass(aCh) == 0) && (BFI(this, BFI_Abbreviations) == 1)) {
    ...
    if ((P >= 0) && (P <= L->Count)) {
      ...
      while ((P > 0) &&
             ((ChClass(L->Chars[P - 1]) == 1) || (L->Chars[P - 1] == '_'))) P--;
      Len = P1 - P;
      C   = ScreenPos(L, P);
      assert(C1 - C == Len);
      ...
      if ((Len > 0) && (Len < int(sizeof(Str)))) {
        ...
        if (ab) {
          ...
          if (ab->Replace != 0) {
            ...
            if (DelText(R, C, C1 - C) == 0) return 0;
            ...
            if (ab->Replace) {
              ...
              if (InsText(R, C, Len, ab->Replace) == 0) return 0;
            ...
            if (((EGUI *)gui)->ExecMacro(View->MView->Win,
                                         ab->Cmd) == 0) return 0;
          }
        }
      }
...
int EBuffer::InsertString(const char *aStr, int aCount) {
  ...
  if (InsText(Y, CP.Col, aCount, aStr) == 0) return 0;
  ...
  if (BFI(this, BFI_WordWrap) == 2) {
    ...
    if (C > BFI(this, BFI_RightMargin)) {
      ...
      while ((C > BFI(this, BFI_LeftMargin)) &&
             ((LP->Chars[P] != ' ') &&
              (LP->Chars[P] != 9))) C = ScreenPos(LP, --P);

      if (P <= BFI(this, BFI_LeftMargin)) {
        C = BFI(this, BFI_RightMargin);
      } else C = ScreenPos(LP, P);

      if (SplitLine(L, C) == 0) return 0;

      IndentLine(L + 1, BFI(this, BFI_LeftMargin));

      if (SetPos(CP.Col - C - 1 +
                 BFI(this, BFI_LeftMargin), CP.Row + 1) == 0) return 0;
    }
  }
  return 1;
}
...
int EBuffer::InsertSpacesToTab(int TSize) {
  ...
  if (TSize <= 0) TSize = BFI(this, BFI_TabSize);

  P1 = NextTab(P, TSize);

  if (BFI(this, BFI_Insert) == 0) {
    if (CP.Col <
        LineLen()) if (DelText(VToR(CP.Row), CP.Col, P1 - P) == 0) return 0;
  }
...
int EBuffer::LineIndentedCharCount(ELine *l, const char *indentchars) {
  ...
  if (!indentchars) indentchars = " \t";
  CC = l->Count;
  PC = l->Chars;

  for (i = 0; i < CC; i++) {
    if (!strchr(indentchars, PC[i])) break;
  }
  return i;
}
...
int EBuffer::IndentLine(int Row, int Indent) {
  ...
  if (Indent != I) {
    if (I > 0) if (DelText(Row, 0, I) == 0) return 0;
    ...
    if (Indent > 0) {
      ...
      if (Indent > 0) if (InsText(Row, C, Indent, 0) == 0) return 0;
    }
  }
  return Ind - I;
}
...
int EBuffer::IsLineBlank(int Row) const {
  ...
  for (P = 0; P < X->Count; P++)
    if ((X->Chars[P] != ' ') && (X->Chars[P] != 9)) return 0;
...
int EBuffer::DoWrap(int WrapAll) {
  ...
  if (L > 0) if (IsLineBlank(L - 1)) FirstParaLine = L;
  ...
  while (L < RCount) {
    ...
    if ((VToR(CP.Row) != L) || (L != FirstParaLine)) {
      if (VToR(CP.Row) == L)
        if (CP.Col <= LineIndented(L)) if (SetPos(Left, CP.Row) == 0) WFAIL(1);
      Ind = IndentLine(L, Left);

      if (VToR(CP.Row) ==
          L) if (SetPos((CP.Col + Ind > 0) ? CP.Col + Ind : 0,
                        CP.Row) == 0) WFAIL(2);
      NoChange = 0;
    }
    Len = LineLen(L);
    ...
    if (Len < Right) {
      ...
      if (firstwordbeg != -1)
        if (firstwordend == -1) firstwordend = lp->Count;
...
int EBuffer::WrapPara() {
  while (VToR(CP.Row) < RCount - 1 &&
         IsLineBlank(VToR(CP.Row))) if (SetPos(CP.Col, CP.Row + 1) == 0) return 0;

  return DoWrap(1);
}

int EBuffer::LineCenter() {
  if (LineTrim() == 0) return 0;
...
int EBuffer::InsPrevLineToEol() {
  ...
  if (L > 0) {
    ...
    if (Len > 0) return InsertString(RLine(L)->Chars + P, Len);
  }
  return 0;
}
...
int EBuffer::TrimLine(int Row) {
  ...
  while ((P > 0) && ((L->Chars[P - 1] == ' ') || (L->Chars[P - 1] == 9))) P--;
  X = ScreenPos(L, P);
...
int EBuffer::ChangeMode(const char *AMode) {
  if ((Mode = FindMode(AMode)) != NULL) {
    ...
    if (Mode->fColorize) HilitProc = GetHilitProc(Mode->fColorize->SyntaxParser);
    FullRedraw();
    return 1;
  }
...
int EBuffer::ChangeFlags(const char *AMode) {
  if (EMode * XMode = FindMode(AMode)) {
    ...
    if (Mode && Mode->fColorize) HilitProc = GetHilitProc(
        Mode->fColorize->SyntaxParser);
    FullRedraw();
    return 1;
  }
  Msg(S_ERROR, "Mode '%s' not found.", AMode);
