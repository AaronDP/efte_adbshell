...
#endif // if defined(NT) && defined(MSVC) && !defined(__WATCOMC__)
...
char* getProgramName(char *name) {
  ...
  if (DosQueryModuleName(pib->pib_hmte, sizeof(ProgramName),
                         ProgramName) != 0) return name;

  return strdup(ProgramName);
}

#endif // if defined(OS2) && defined(__EMX__)
...
# endif // ifdef UNIX
...
#else // ifndef FTE_NO_LOGGING
          printf("--debug, --debugclean disabled\n");
#endif // ifndef FTE_NO_LOGGING
        } else QuoteAll = 1;
      } else if (argv[Arg][1] == '!') {
        ign = 1;
      } else if (argv[Arg][1] == '+') {
  ...
  } else strcpy(ConfigFileName, "mymain.fte");
  ...
  if (LoadDesktopOnEntry == 2) LoadDesktopOnEntry = 1;
  return 1;
}
...
int main(int argc, char **argv) {
...
#endif // if defined(__EMX__) || (defined(NT) && defined(MSVC) &&
       // !defined(__WATCOMC__))
  ...
  if (getuid() != effuid) seteuid(getuid());

  if (getgid() != effgid) setegid(getgid());
#endif // if defined(UNIX) && defined(LINUX)
  ...
  if (CmdLoadConfiguration(argc, argv) == 0) return 1;

  STARTFUNC("main");

  EGUI *g = new EGUI(argc, argv, ScreenSizeX, ScreenSizeY);

  if ((gui == 0) || (g == 0)) DieError(1, "Failed to initialize display\n");
  ...
  _dump_allocated(64);
#endif // if defined(__DEBUG_ALLOC__)
