...
// typedef int (*SyntaxProc)(EBuffer *BF, int LN, PCell B, int Pos, int Width,
// ELine *Line, hlState &State, hsState *StateMap);
typedef HILIT_P ((*SyntaxProc));


int Indent_Plain(EBuffer *B,
                 int      Line,
                 int      PosCursor);
int Indent_Continue(EBuffer *B,
                    int      Line,
                    int      PosCursor);
...
int Indent_C(EBuffer *B,
             int      Line,
             int      PosCursor);
int Indent_REXX(EBuffer *B,
                int      Line,
                int      PosCursor);
int Indent_SIMPLE(EBuffer *B,
                  int      Line,
                  int      PosCursor);
...
#define NextChar() \
  do { i++; p++; len--; C++; } while (0)
#define ColorNext() \
  do { ColorChar(); NextChar(); } while (0)
...
public:
  ...
  EColorize(const char *AName,
            const char *AParent);
  ~EColorize();

  int SetColor(int         clr,
               const char *value);
};
