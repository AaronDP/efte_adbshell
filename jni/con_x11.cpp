...
#endif // ifdef USE_XICON

#ifdef CAST_FD_SET_INT
# define FD_SET_CAST() (int *)
#else // ifdef CAST_FD_SET_INT
# define FD_SET_CAST()
#endif // ifdef CAST_FD_SET_INT
...
static void SendSelection(XEvent        *notify,
                          Atom           property,
                          Atom           type,
                          unsigned char *data,
                          int            len,
...
static int InitXColors() {
  ...
  for (i = 0; i < 16; i++) {
    ...
    if (XAllocColor(display, colormap, &Colors[i]) == 0) {
      ...
      /*colormap = XCreateColormap(display, win, DefaultVisual(display, screen),
         AllocNone);
         for (i = 0; i < 16; i++) {
         SetColor(i);
...
static void try_fontset_load(const char *fs) {
  ...
  if (fontSet == NULL) {
    ...
    for (int i = 0; i < nMiss; i++) fprintf(stderr, "  %s\n", miss[i]);

    if (def != NULL) fprintf(stderr, " def_ret: %s\n", def);
  }
}
...
static int InitXFonts(void) {
  ...
  if ((fs == NULL) && (WindowFont[0] != 0)) fs = WindowFont;
  ...
  if (!useXMB) {
    ...
    if (fontStruct == NULL) fontStruct = XLoadQueryFont(display, "8x13");

    if (fontStruct == NULL) fontStruct = XLoadQueryFont(display, "fixed");

    if (fontStruct == NULL) return -1;
...
static int SetupXWindow(int argc, char **argv) {
  ...
  if ((display = XOpenDisplay(display_name)) == NULL) {
    DieError(1,
             "XeFTE Fatal: could not open display: %s!",
...
// but we need to open a window - so pick up 1 x 1
  ...
  sizeHints.flags = PResizeInc | PMinSize | PMaxSize | PBaseSize |
                    PWinGravity;
  ...
  if (setUserPosition) sizeHints.flags |= USPosition;
  ...
  if (XpmCreatePixmapFromData(display, win, const_cast<char **>(efte16x16_xpm),
  ...
  for (i = 0; i < ICON_COUNT; i++) {
    ...
    if (XpmCreateXpmImageFromData(const_cast<char **>(xpmData[i]), &xpm,
                                  NULL) != XpmSuccess) break;
    iconBufferSize += 2 + xpm.width * xpm.height;
    colors          = (CARD32 *)malloc(xpm.ncolors * sizeof(CARD32));

    if (colors == NULL) {
      // Need to clear here is cleanup at the end checks for colors[i] to see if
      // XPM was loaded
      XpmFreeXpmImage(&xpm);
      break;
    }
...
int ConInit(int XSize, int YSize) {
  if (XSize != -1) ScreenCols = XSize;

  if (YSize != -1) ScreenRows = YSize;
...
void DrawCursor(int Show) {
  if (CursorVisible) {
    ...
    if (CursorBlink) Show &= (CursorLastTime % (CursorFlashInterval * 2)) >
                             CursorFlashInterval;
    int attr = p[1];

    if (Show) attr ^= insertState ? CursorInsertMask : CursorOverMask;
...
int ConScroll(int Way, int X, int Y, int W, int H, TAttr Fill, int Count) {
  ...
  if (Way == csUp) {
    ...
    for (l = 0; l < H - Count;
         l++) memcpy(CursorXYPos(X, Y + l), CursorXYPos(X, Y + l + Count), 2 * W);

    if (ConSetBox(X, Y + l, W, Count, Cell) == -1) return -1;
  } else if (Way == csDown) {
    XCopyArea(display, win, win, GCs[0],
              X * FontCX,
              Y * FontCY,
...
void ConvertKeyToEvent(KeySym  key,
                       KeySym  key1,
                       char */*keyname*/,
                       char */*keyname1*/,
                       int     etype,
  ...
  if ((key < 256) ||
      ((key1 < 256) && ((myState == kfAlt) || (myState == (kfAlt | kfShift))))) {
    if (myState & kfAlt) key = key1;

    if (myState == kfShift) myState = 0;

    if (myState & (kfAlt | kfCtrl))
      if ((key >= 'a') && (key < 'z' + 32)) key &= ~0x20;

    if ((myState & kfCtrl) && (key < 32)) key += 64;
...
#define TM_DIFF(x,                                                              \
                y) ((long)(((long)(x) < (long)(y)) ? ((long)(y) -               \
                                                      (long)(x)) : ((long)(x) - \
                                                                    (           \
                                                                      long)(y))))

void ConvertClickToEvent(int     type,
                         int     xx,
                         int     yy,
                         int     button,
                         int     state,
    ...
    case Button5:
      ...
      if (type == ButtonPress) {
        ...
        if (state & ShiftMask) {
          ...
          if (button == Button4) Event->Msg.Command = cmVScrollUp;  // fix core
                                                                    // to use
                                                                    // count
          else Event->Msg.Command = cmVScrollDown;
        } else {
          Event->Msg.Param1 = 3;

          if (button == Button4) Event->Msg.Command = cmVScrollUp;
          else Event->Msg.Command = cmVScrollDown;
        }
      }
      return;
...
void ProcessXEvents(TEvent *Event) {
  ...
  if (anyEvent->window != win) {
    if ((event.type == PropertyNotify) &&
      ...
      // Property change on different window - try to find matching incremental
      // selection request
  ...
  case ConfigureNotify:
    ...
    while ((XPending(display) > 0) &&
           XCheckTypedWindowEvent(display, win,
                                  ConfigureNotify, &event)) XSync(display, 0);
  ...
  case ButtonRelease:
    ...
    ConvertClickToEvent(event.type,
                        buttonEvent->x,
                        buttonEvent->y,
                        buttonEvent->button,
                        buttonEvent->state,
  ...
  case KeyPress:
    ...
    if (!i18n_ctx || (event.type == KeyRelease)) XLookupString(keyEvent,
                                                               keyName,
                                                               sizeof(keyName),
                                                               &key,
                                                               0);
  ...
  case SelectionClear: {
    ...
    if (clip >= 0) {
      ...
      if (owner != win) {
        if (CurSelectionData[clip] != NULL) free(CurSelectionData[clip]);
  ...
  case SelectionRequest: {
    ...
    //   property  - target property - place data to this property of
    // requestor's window, set correct type
    //   requestor - selection requestor
    //   time      - request time - owner should provide selection if it owned
    // it at this time
    //  Note: Old clients use None property - in this case the right property is
    // stored in target field.
    ...
    //   target    - -- copy --
    //   property  - -- copy -- or None if on error - request could not be
    // fulfilled
    //   time      - -- copy --
...
# endif // ifdef X_HAVE_UTF8_STRING
        ...
        XChangeProperty(display,
                        ...
                        XA_ATOM,
                        32, PropModeReplace,
                        (unsigned char *)&type_list, sizeof(type_list) /
                        sizeof(*type_list));
        ...
        // No conversion, just the string we have (in fact we should convert to
        // ISO Latin-1)
        ...
        char *text_list[1] =
        { (char *)(CurSelectionData[clip] ? CurSelectionData[clip] : empty) };
...
# endif // ifdef X_HAVE_UTF8_STRING
        ...
        if (style != -1) {
          // Can convert
          if (XmbTextListToTextProperty(display, text_list, 1, style,
                                        &text_property) == Success) {
            if (text_property.format == 8) {
              // SendSelection supports only 8-bit data (should be always, just
              // safety check)
              SendSelection(&notify,
                            event.xselectionrequest.property,
    ...
    if (!notifySent) XSendEvent(display,
...
static void FlashCursor() {
  ...
  if (!CursorBlink || (gettimeofday(&tv, NULL) != 0)) return;

  unsigned long OldTime = CursorLastTime;
  CursorLastTime = tv.tv_sec * 1000 + tv.tv_usec / 1000;

  if (OldTime / CursorFlashInterval != CursorLastTime /
      CursorFlashInterval) DrawCursor(CursorVisible);
}
...
int ConGetEvent(TEventMask EventMask, TEvent *Event, int WaitTime, int Delete) {
  ...
  while (Event->What == evNone) {
    ...
    for (int p = 0; p < MAX_PIPES; p++)
      if (Pipes[p].used)
        if (Pipes[p].fd != -1) FD_SET(Pipes[p].fd, &read_fds);

    if (((WaitTime == -1) ||
         (WaitTime > MouseAutoDelay)) && (LastMouseEvent.What == evMouseAuto) &&
        (EventMask & evMouse)) {
      timeout.tv_sec  = 0;
...
static int ConvertSelection(Atom selection, Atom type, int *len, char **data) {
  ...
  if (!WaitForXEvent(SelectionNotify,
                     &event) ||
      (event.xselection.property != prop_selection)) return -1;
  ...
  if (actual_type == proptype_incr) {
    ...
    // Cannot exit right now if data == NULL since we need to complete the
    // handshake
  ...
  if (actual_type == XA_STRING) {
    ...
    // This propagation is not safe since it expects XFree() to be the same as
    // free().
    // Rather we should make a copy of the received data. The similar applies to
    // data
    // propagated from INCR branch above - they are allocated by malloc() but
    // get freed
...
# endif // ifdef X_HAVE_UTF8_STRING
      ...
      if (ConvertSelection(clip, proptype_compound_text, len,
                           data) == 0) return 0;
...
int SetXSelection(int len, char *data, int clipboard) {
  ...
  if (CurSelectionData[clipboard] != NULL) free(CurSelectionData[clipboard]);

  // We need CurSelectionData zero-terminated so XmbTextListToTextProperty can
  // work
  ...
  if (XGetSelectionOwner(display, clip) == win) CurSelectionOwn[clipboard] = 1;
  return 0;
}
...
GUI::GUI(int  & argc,
  ...
  for (int c = 1; c < argc; c++) {
    ...
    if (strcmp(argv[c], "-geometry") == 0) {
      if (c + 1 < argc) {
        ...
        if (ScreenCols > ConMaxCols) ScreenCols = ConMaxCols;
        else if (ScreenCols < MIN_SCRWIDTH) ScreenCols = MIN_SCRWIDTH;

        if (ScreenRows > ConMaxRows) ScreenRows = ConMaxRows;
        else if (ScreenRows < MIN_SCRHEIGHT) ScreenRows = MIN_SCRHEIGHT;
        XSize           = YSize = -1;
        setUserPosition = 1;
      }
    } else
  ...
  if ((::ConInit(XSize,
                 YSize) == 0) && (SetupXWindow(argc, argv) == 0)) gui = this;
  else gui = NULL;
...
GUI::~GUI() {
  ...
  for (int i = 0; i < 256; i++) XFreeGC(display, GCs[i]);
#ifdef USE_XMB

  if (fontSet) XFreeFontSet(display, fontSet);
#endif // ifdef USE_XMB

  if (fontStruct) XFreeFont(display, fontStruct);
...
#else // ifndef NO_PIPES
  return 0;

#endif // ifndef NO_PIPES
}
...
int GUI::ClosePipe(int id) {
  ...
  if (Pipes[id].fd != -1) close(Pipes[id].fd);
...
int GUI::RunProgram(int mode, char *Command) {
  ...
  if (*Command == 0) // empty string = shell
    ...
    if (mode == RUN_ASYNC) strlcat(Cmd, " &", sizeof(Cmd));
  }
  return system(Cmd);
}
