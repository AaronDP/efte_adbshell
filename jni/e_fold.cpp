...
int EBuffer::FindNearFold(int Line) { /*FOLD00*/
  ...
  while (b <= B) {
    ...
    if (FF[c].line == Line) return c;

    if (c < FCount - 1) {
      if ((FF[c].line <= Line) && (FF[c + 1].line > Line)) return c;
    } else {
      if (FF[c].line <= Line) return c;
    }

    if (FF[c].line < Line) b = c + 1;
    else B = c - 1;
...
void EBuffer::UpdateVis(EPoint& M, int Row, int Delta) { /*FOLD00*/
  if (Delta < 0) {
    if (M.Row > Row) {
      if (M.Row < Row - Delta) M.Row = Row;
      else M.Row += Delta;
    }
  } else {
    if (M.Row >= Row) M.Row += Delta;
  }
}
...
int EBuffer::FoldCreateAtRoutines() { /*FOLD00*/
  if (BFS(this, BFS_RoutineRegexp) == 0) return 0;
...
int EBuffer::FoldDestroyAll() { /*FOLD00*/
  ...
  for (l = 0; l < RCount; l++)
    if (FindFold(l) != -1) if (FoldDestroy(l) == 0) return 0;
...
int EBuffer::FoldOpenAll() { /*FOLD00*/
  ...
  for (l = 0; l < RCount; l++)
    if (FindFold(l) != -1) if (FoldOpen(l) == 0) return 0;
...
int EBuffer::FoldCloseAll() { /*FOLD00*/
  ...
  for (l = RCount - 1; l >= 0; l--)
    if (FindFold(l) != -1) if (FoldClose(l) == 0) return 0;
...
int EBuffer::MoveFoldTop() { /*FOLD00*/
  ...
  if (FF[f].line == VToR(CP.Row)) return 1;
