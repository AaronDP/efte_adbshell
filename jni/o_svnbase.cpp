...
int AddSvnIgnoreRegexp(const char *regexp) {
  ...
  if ((SvnIgnoreRegexp[SvnIgnoreRegexpCount] = RxCompile(regexp)) ==
      NULL) return 0;
...
ESvnBase::ESvnBase(int         createFlags,
                   EModel    **ARoot,
...
void ESvnBase::AddLine(const char *file, int line, const char *msg, int status) {
  ...
  if (l != 0) {
    ...
    LineCount++;
    Lines                = (SvnLine **)realloc(Lines,
                                               sizeof(SvnLine *) * LineCount);
    Lines[LineCount - 1] = l;
...
void ESvnBase::NotifyPipe(int APipeId) {
  if (APipeId == PipeId) {
    ...
    while (GetLine((char *)line, sizeof(line))) {
      ...
      for (i = 0; i < SvnIgnoreRegexpCount;
           i++) if (RxExec(SvnIgnoreRegexp[i], line, len, line, &RM) == 1) break;
...
int ESvnBase::RunPipe(const char *ADir,
                      const char *ACommand,
                      const char *AOnFiles) {
  free(Command);
  free(Directory);
...
void ESvnBase::GetTitle(char *ATitle, int MaxLen, char *ASTitle,
                        int SMaxLen) const {
  char format[128];
