...
EFrame::EFrame(int XSize,
               int YSize) : GFrame(XSize, YSize) {
  CMap   = 0;
  CModel = 0;
}
...
void EFrame::UpdateMenu() {
  ...
  if ((Map != CMap) || (CMap == 0)) {
    ...
    if (CMap) OMenu = CMap->GetMenu(EM_MainMenu);

    if (Map) NMenu = Map->GetMenu(EM_MainMenu);

    if (NMenu == 0) NMenu = "Main";
    CMap = Map;
...
int EGUI::ExecCommand(GxView *view, int Command, ExState& State) {
  if (Command & CMD_EXT) return ExecMacro(view, Command & ~CMD_EXT);

  if (Command == ExFail) return ErFAIL;
  ...
  case ExDesktopSave:
    ...
    if (DesktopFileName[0] != 0) return SaveDesktop(DesktopFileName);
...
void EGUI::SetMsg(const char *Msg) {
  ...
  if (ActiveModel) ActiveModel->Msg(S_INFO, CharMap);
}
...
void EGUI::DispatchKey(GxView *view, TEvent& Event) {
  ...
  if (Event.Key.Code & kfModifier) return;

  LastEventChar = -1;

  if (GetCharFromEvent(Event, &Ch)) LastEventChar = Ch;

  if ((EventMap = view->GetEventMap()) == 0) return;
  ...
  if (ActiveMap || OverrideMap) {
    ...
    if (OverrideMap) map = OverrideMap;
...
void EGUI::DispatchEvent(GFrame *frame, GView *view, TEvent& Event) {
  ...
  if ((Event.What == evNone) ||
      ((Event.What == evMouseMove) && (Event.Mouse.Buttons == 0))) return;
  ...
  if (_heapchk() != _HEAPOK) DieError(0, "Heap memory is corrupt.");
#endif // if defined(OS2) && !defined(DBMALLOC) && defined(CHECKHEAP)
}
...
int EGUI::FileCloseX(EView *View, int CreateNew, int XClose) {
  ...
  if (GetDefaultDirectory(View->Model, Path, sizeof(Path)) == 0) return 0;
  ...
  if (View->Model->ConfQuit(View->MView->Win)) {
    ...
    if (XClose)
      while (View->Model->Next != View->Model &&
             View->Model->Next->CanQuit()) delete View->Model->Next;
...
int EGUI::FileClose(EView *View, ExState& State) {
  ...
  if (State.GetIntParam(View, &x) == 0) x = OpenAfterClose;
...
int EGUI::FileCloseAll(EView *View, ExState& State) {
  ...
  while (ActiveModel) if (FileCloseX(View, x, 1) == 0) return 0;
...
int EGUI::WinResize(ExState& State, GxView *View) {
  ...
  if (State.GetIntParam(0, &Delta)) {
    if (View->ExpandHeight(Delta) == 0) return 1;
  }
  return 0;
}
...
int EGUI::ExitEditor(EView *View) {
  ...
  while (ActiveModel) {
    ...
    if (ActiveModel == B) break;
  }

  if (SaveDesktopOnExit && (DesktopFileName[0] != 0)) SaveDesktop(DesktopFileName);
  ...
  while (ActiveModel) {
    if (View->Model->GetContext() == CONTEXT_ROUTINES) { // Never delete Routine
                                                         // models directly
      ActiveModel = ActiveModel->Next;
      View->SelectModel(ActiveModel);
    }
...
int EGUI::RunProgram(ExState& State, GxView *view) {
  ...
  if (ActiveModel) SetDefaultDirectory(ActiveModel);

  if (State.GetStrParam(ActiveView, Cmd,
                        sizeof(Cmd)) ==
      0) if (view->GetStr("Run", sizeof(Cmd), Cmd, HIST_COMPILE) == 0) return 0;
...
int EGUI::MainMenu(ExState& State, GxView *View) {
  ...
  if (State.GetStrParam(0, s, sizeof(s)) == 0) s[0] = 0;
...
int EGUI::ShowMenu(ExState& State, GxView *View) {
  ...
  if (State.GetStrParam(0, MName, sizeof(MName)) == 0) return 0;
...
int EGUI::DesktopSaveAs(ExState& State, GxView *view) {
  if (State.GetStrParam(0, DesktopFileName, sizeof(DesktopFileName)) == 0)
    if (view->GetFile("Save Desktop", sizeof(DesktopFileName), DesktopFileName,
                      HIST_PATH, GF_SAVEAS) == 0) return 0;
...
int EGUI::DesktopLoad(ExState& State, GxView *view) {
  if (State.GetStrParam(0, DesktopFileName, sizeof(DesktopFileName)) == 0)
    if (view->GetFile("Load Desktop", sizeof(DesktopFileName), DesktopFileName,
                      HIST_PATH, GF_OPEN) == 0) return 0;

  if (DesktopFileName[0] != 0) return LoadDesktop(DesktopFileName);
...
int EGUI::FrameClose(GxView *View) {
  ...
  if (!frames->IsLastFrame()) {
    ...
    if (ExitEditor(ActiveView) == 0) return 0;
...
int EGUI::FindDesktop(char *argv[]) {
  ...
  default:
    ...
    if (FileExists(DESKTOP_NAME)) ExpandPath(DESKTOP_NAME,
                                             DesktopFileName,
...
void EGUI::DoLoadDesktopOnEntry(int& /*argc*/, char **argv) {
  if (DesktopFileName[0] == 0) FindDesktop(argv);
  ...
  if (DesktopFileName[0] != 0) {
    ...
    if (LoadDesktopOnEntry && FileExists(DesktopFileName)) LoadDesktop(
        DesktopFileName);
  }
}
...
int EGUI::InterfaceInit(int& /*argc*/, char ** /*argv*/) {
  if (FrameNew() == 0) DieError(1, "Failed to create window\n");
  return 0;
}
...
void EGUI::DoLoadHistoryOnEntry(int& /*argc*/, char **argv) {
  ...
  if (KeepHistory && FileExists(HistoryFileName)) LoadHistory(HistoryFileName);
}

void EGUI::DoSaveHistoryOnExit() {
  if (KeepHistory && (HistoryFileName[0] != 0)) SaveHistory(HistoryFileName);
  ...
  for (int Arg = 1; Arg < argc; Arg++) {
    if (!QuoteAll && !QuoteNext && (argv[Arg][0] == '-')) {
      ...
      case '-':
        ...
        if (strncmp(argv[Arg], "--debug", 7) != 0) QuoteAll = 1;
        Arg = argc;
        break;
      ...
      if ((ExpandPath(argv[Arg], Path,
                      sizeof(Path)) == 0) && IsDirectory(Path)) {
        ...
        if (GotoLine) {
          if (((EBuffer *)ActiveModel)->Loaded ==
              0) ((EBuffer *)ActiveModel)->Load();
          ...
          if (GotoLine) {
            ...
            if (RetrieveFPos(((EBuffer *)ActiveModel)->FileName, r,
                             c) == 1) ((EBuffer *)ActiveModel)->SetNearPosR(c, r);
          }
...
int EGUI::Start(int& argc, char **argv) {
  ...
  if (InterfaceInit(argc, argv) != 0) return 2;
  ...
  if (CmdLoadFiles(argc, argv) == 0) return 3;
...
void EGUI::EditorCleanup() {
  ...
  if (ActiveView != NULL) {
    ...
    // If EView what is about to be deleted is currently ActiveView, ActiveView
    // moves to next one
  ...
  if (Macros != 0) {
    while (CMacros--) {
      ...
      for (int i = 0; i < Macros[CMacros].Count; ++i)
        if (Macros[CMacros].cmds[i].type ==
            CT_STRING) free(Macros[CMacros].cmds[i].u.string);
