...
# elif defined(NT)
  {
    ...
    if (!GetCurrentDirectory(sizeof(orig), orig)) return -1;

    newdrive[0] = drive + 'A' - 1;
    newdrive[1] = ':';
    newdrive[2] = 0;

    if (!SetCurrentDirectory(newdrive)) return -1;

    if (!GetCurrentDirectory(sizeof(newdrive), newdrive)) return -1;
...
int RemoveDots(const char *Source, char *Dest) {
  ...
  while (*p) {
    ...
    if (ISSLASH(p[0]) && ISSLASH(p[1])) {
      ...
      // } else if ((strncmp(p, SSLASH ".." SSLASH, 4) == 0) || (strcmp(p,
      // SSLASH "..") == 0)) {
      ...
      *d = 0;
      ...
      // } else if ((strncmp(p, SSLASH "." SSLASH, 3) == 0) || (strcmp(p, SSLASH
      // ".") == 0)) {
...
int ExpandPath(const char *Path, char *Expand, int ExpandSize) {
  ...
  if (Name[0] != 0) slashed = ISSLASH(Name[strlen(Name) - 1]);
  Slash(Name, 0);
  ...
  case '~': {
    ...
    if ((Name[1] == SLASH) || (Name[1] == 0)) {
      path = Name + 1;
      char *homePath = getenv("HOMEPATH");                          // Windows
                                                                    // Vista

      if (homePath == NULL) homePath = getenv("USERPROFILE");       // Windows
                                                                    // XP

      if (homePath == NULL) strlcpy(Name2, "C:\\", sizeof(Name2));  // Windows
                                                                    // 95/98 do
                                                                    // not have
                                                                    // home path
      else strlcpy(Name2, homePath, sizeof(Name2));
    }

    if (path[0] != SLASH) Slash(Name2, 1);
  ...
  if (Name[0] && (Name[1] == ':') && (Name[2] == 0)) { // '?:'
    ...
    if (GetDiskCurDir(drive, Expand + 3) == -1) return -1;
  } else {
# if defined(__EMX__)
...
# if defined(__EMX__)
  {
    ...
    if (p && *p) do {
        if (ISSLASH(*p)) *p = SLASH;
      } while (*p++);
  }
# endif // if defined(__EMX__)

  if (slashed) SlashDir(Expand);
  return 0;
...
int JustDirectory(const char *Path, char *Dir, int DirSize) {
  ...
  if (ExpandPath(Path, Dir, DirSize) == -1) strlcpy(Dir, Path, DirSize);
  p = SepRChr(Dir);
...
int JustLastDirectory(const char *Path, char *Dir, int DirSize) {
  ...
  while (secondLastSlash > 0 &&
         !ISSEP(Path[secondLastSlash - 1])) secondLastSlash--;
...
int ChangeDir(char *Dir) {
  ...
  if (Dir[0] && (Dir[1] == ':'))
    if (SetDrive(toupper(Dir[0]) - 'A' + 1) == -1) return -1;
...
char* SepRChr(char *Dir) {
  ...
  if (Dir && Dir[0]) {
    for (p = Dir + strlen(Dir); p > Dir; p--)
      if (ISSEP(p[-1])) return p - 1;
  }
  return NULL;
}

// Returns relative path of Path with respect to Dir
// Ex: Dir = /home/martin/src/ Path = /home/martin/src/Fte/s_files.cpp ->
// Fte/s_files.cpp
int RelativePathName(const char *Dir,
                     const char *Path,
                     char       *RelPath,
                     int         RelPathSize) {
