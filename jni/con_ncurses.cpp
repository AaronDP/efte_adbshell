      ...
      for (int fgb = 0; fgb < 2; fgb++) { /* fg bright bit */
        for (int fg = 0; fg < 8; fg++, c++) {
          if (colors) {
            ...
            if (c !=
                0) init_pair(pair, fte_curses_colors[fg], fte_curses_colors[bg]);
...
int ConInit(int /*XSize */, int /*YSize */) {
  ...
  for (ch = KEY_MAX + 1;; ch++) {
    ...
    if (!strcmp(s, "kUP")) key_sup = ch;
    else if (!strcmp(s, "kDN")) key_sdown = ch;
...
int ConPutBox(int X, int Y, int W, int H, PCell Cell) {
  ...
  for (int j = 0; j < H; j++) {
    memcpy(SavedScreen[Y + j] + X, Cell, W * sizeof(TCell)); // copy outputed
                                                             // line to saved
                                                             // screen
    move(yy++, X);
...
extern int WaitPipeEvent(TEvent *Event,
                         int     WaitTime,
                         int    *fds,
                         int     nfds);
...
int ConGetEvent(TEventMask /*EventMask */,
  ...
  if (ch < 0) {
    ...
    default:
      ...
      if ((key_sdown != 0) && (ch == key_sdown)) KEvent->Code = kfShift | kbDown;
      else if ((key_sup != 0) && (ch == key_sup)) KEvent->Code = kfShift | kbUp;
      else {
        Event->What = evNone;
