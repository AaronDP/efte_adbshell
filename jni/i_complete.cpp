...
ExComplete::ExComplete(EBuffer *B) : ExView() {
  ...
  if (Words != NULL) RefreshComplete();
}

ExComplete::~ExComplete() {
  //    fprintf(stderr, "W %p %p %p %d\n", Words, WordContinue, WordBegin,
  // WordsLast);

  if (WordBegin != NULL) delete[] WordBegin;

  if (Words != NULL) {
    for (int i = 0; i < WordsLast; i++) delete[] Words[i];
    delete[] Words;
  }
}
...
void ExComplete::HandleEvent(TEvent& Event) {
  ...
  if (WordsLast < 2) {
    ...
    case kbHome:
      ...
      for (i = 0; i < WordPos; i++)
        if (strncmp(Words[WordPos], Words[i], WordFixed) == 0) WordPos = i;
      Event.What = evNone;
      break;
    ...
    default:
      ...
      if (CheckASCII(Event.Key.Code & ~kfShift)) {
        ...
        if (s != NULL) {
          if (WordFixed > 0) strncpy(s, Words[WordPos], WordFixed);
  ...
  if (DoQuit) {
    /* int rc = 0;
       ...
       if (Buffer->InsText(Buffer->VToR(Orig.Row), Orig.Col, l, Words[WordPos],
          1)
         && Buffer->SetPos(Orig.Col + l, Orig.Row)) {
         Buffer->Draw(Buffer->VToR(Orig.Row), Buffer->VToR(Orig.Row));
         rc = 1;
...
int ExComplete::RefreshComplete() {
  if ((Buffer->CP.Col == 0)
      || (Buffer->SetPos(Buffer->CP.Col, Buffer->CP.Row) == 0)) return 0;

  PELine L = Buffer->VLine(Buffer->CP.Row);
  int    C = Buffer->CP.Col;
  int    P = Buffer->CharOffset(L, C);

  if (!P || (P > L->Count)) return 0;

  int P1 = P;

  while ((P > 0) && CheckASCII(L->Chars[P - 1])) P--;
  ...
  while (Buffer->FindStr(L->Chars + P, wlen, mask) == 1) {
    ...
    if ((L->Chars == M->Chars) && (P == X)) continue;

    int XL = X;

    while ((XL < M->Count) && CheckASCII(M->Chars[XL])) XL++;
    ...
    if (s != NULL) {
      ...
      if (c != 0) {
        ...
        if (WordsLast >= MAXCOMPLETEWORDS) break;
      } else {
...
void ExComplete::FixedUpdate(int add) {
  if (add < 0) {
    if (WordFixed > 0) WordFixed += add;
  } else if (add > 0) {
    if (strlen(Words[WordPos]) > WordFixed) WordFixed += add;
  }
  ...
  if (WordFixed > 0) {
    ...
    for (int i = 0; i < WordsLast; i++)
      if (strncmp(Words[WordPos], Words[i], WordFixed) == 0) WordFixedCount++;
  } else WordFixedCount = WordsLast;
}
