...
int MakeTrans(TransTable tab, int What) {
  ...
  // no translation
  for (i = 0; i <= 255; i++) tab[i] = (unsigned char)i;
  ...
  case ccUp:
    ...
    for (i = 33; i <= 255; i++)
      if (isalpha(i) && (toupper(i) != i)) tab[i] = (unsigned char)toupper(i);

    if (What != ccToggle) break;

  case ccDown:

    for (i = 33; i <= 255; i++)
      if (isalpha(i) && (i == tab[i]) &&
          (tolower(i) != i)) tab[i] = (unsigned char)tolower(i);
    break;
...
int EBuffer::BlockTrans(TransTable tab) {
  ...
  for (L = BB.Row; L <= BE.Row; L++) {
    ...
    if (B > LL->Count) B = LL->Count;

    if (E > LL->Count) E = LL->Count;

    if (E > B) {
      if (ChgChars(L, B, E - B, 0) == 0) return 0;

      for (I = B; I < E; I++) LL->Chars[I] = tab[(unsigned char)LL->Chars[I]];
    }
  }
  Draw(BB.Row, BE.Row);
  return 1;
...
int EBuffer::LineTrans(TransTable tab) {
  ...
  if (L->Count > 0) {
    ...
    for (I = 0; I < L->Count; I++) L->Chars[I] = tab[(unsigned char)L->Chars[I]];
  }
  return 1;
}
...
int EBuffer::GetTrans(ExState& State, TransTable tab) {
  ...
  if (State.GetStrParam(View, (char *)TrS, sizeof(TrS)) == 0)
    if (View->MView->Win->GetStr("Trans From", sizeof(TrS), (char *)TrS,
                                 HIST_TRANS) == 0) return 0;

  if (State.GetStrParam(View, (char *)TrD, sizeof(TrD)) == 0)
    if (View->MView->Win->GetStr("Trans To", sizeof(TrS), (char *)TrD,
                                 HIST_TRANS) == 0) return 0;
...
int EBuffer::CharTrans(ExState& State) {
  ...
  if (GetTrans(State, tab) == 0) return 0;
