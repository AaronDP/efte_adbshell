...
HMQ  (APIENTRY *p_WinCreateMsgQueue)(HAB  hab,
                                     LONG cmsg);
...
BOOL (APIENTRY *p_WinSetClipbrdData)(HAB   hab,
                                     ULONG ulData,
                                     ULONG fmt,
                                     ULONG rgfFmtInfo);
ULONG (APIENTRY *p_WinQueryClipbrdData)(HAB   hab,
                                        ULONG fmt);
...
static BOOL loadDLL(void) {
  ...
  if (loaded == TRUE) return loaded_ok;
  ...
  if (DosLoadModule((PSZ)error, sizeof(error), (PSZ)"PMWIN.DLL", &pmwin) == 0) {
    ...
    for (imp = imported_functions; imp->ordinal; imp++)
      if (DosQueryProcAddr(pmwin, imp->ordinal, NULL,
                           imp->pointer) != 0) return FALSE;
...
static void LeavePmClipboard(void) {
  if (PmInfo.opened) p_WinCloseClipbrd(PmInfo.hab);

  if (PmInfo.hmq) p_WinDestroyMsgQueue(PmInfo.hmq);

  if (PmInfo.hab) p_WinTerminate(PmInfo.hab);
...
int GetClipText(ClipData *cd) {
  ...
  if (AccessPmClipboard() != TRUE) return rc;
...
int PutClipText(ClipData *cd) {
  ...
  if (len) {
    ...
    if (!p_WinSetClipbrdData(PmInfo.hab, (ULONG)text, CF_TEXT,
                             CFI_POINTER)) DosBeep(100, 1500);
    else rc = 0;
  }
