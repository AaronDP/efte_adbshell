...
int Hilit_MSG(EBuffer *BF,
  ...
  if (Line->Count > 0) {
    ...
    if (State == hsMSG_Normal) {
      if ((Line->Count >= 2) &&
          (Line->Chars[0] == '-') &&
          (Line->Chars[1] == '-') &&
          ((Line->Count == 2) || (Line->Chars[2] == ' '))) is_tag = 1;
               ...
               (Line->Chars[1] == '-') &&
               (Line->Chars[2] == '-') &&
               ((Line->Count == 3) || (Line->Chars[3] == ' '))) is_tag = 1;
      else if ((Line->Count >= 3) &&
               (Line->Chars[0] == '.') &&
               (Line->Chars[1] == '.') &&
               (Line->Chars[2] == '.') &&
               ...
               (memcmp(Line->Chars, " * Origin:", 10) == 0)) is_control = 1;
      else if ((Line->Count > 0) && (Line->Chars[0] == '\x01')) is_control = 1;
  ...
  if (ExpandTabs) { /* use slow mode */
    ...
    if (Pos < Line->Count) {
      if (Pos + Width < Line->Count) {
        if (B) MoveMem(B, 0, Width, Line->Chars + Pos, Color, Width);

        if (StateMap) memset(StateMap, State, Line->Count);
      } else {
        if (B) MoveMem(B, 0, Width, Line->Chars + Pos, Color, Line->Count - Pos);
  ...
  if (State != hsMSG_Header) State = hsMSG_Normal;
  *ECol = C;
  return 0;
}
